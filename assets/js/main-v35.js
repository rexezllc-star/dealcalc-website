...
/* ===== DealCalc V25.1 Hotfix =====
   Roll forward stable V24 parser, fix gross rent yield fallback, and sort Best Use by displayed score.
   This avoids the V25 PDF.js regression while preserving underwriting improvements. */
function dcV251Number(v){
  if(typeof v==='number') return isFinite(v)?v:0;
  if(typeof v==='string') return Number(v.replace(/[^0-9.-]/g,''))||0;
  return 0;
}
function dcV251RentPrice(x){
  const meta=getDetectedMeta ? getDetectedMeta() : {};
  const rent=dcV251Number(x?.rent)||dcV251Number(meta.monthlyRent)||dcV251Number(document.getElementById('monthlyRent')?.value);
  const price=dcV251Number(x?.listing)||dcV251Number(x?.price)||dcV251Number(meta.listingPrice)||dcV251Number(document.getElementById('listingPrice')?.value)||dcV251Number(document.getElementById('purchasePrice')?.value);
  return {rent,price};
}
function dcV24GrossYield(x){
  const {rent,price}=dcV251RentPrice(x||{});
  return rent && price ? safeDiv(rent*12,price) : 0;
}
function scoreRetail(x){
  const meta=getDetectedMeta();
  const rp=dcV251RentPrice(x||{});
  const price=dcV251Number(x.listing)||dcV251Number(x.price)||rp.price;
  const value=dcV251Number(x.value)||dcV251Number(meta.underwritingValue)||price;
  const premium=safeDiv(price-value,value);
  const equity=meta.estimatedEquity || (value-(dcV251Number(x.loan)||dcV251Number(meta.mortgageBalance)||0));
  const yieldPct=dcV24GrossYield(x);
  let score=58-(premium*70);
  if(/Updated|Ready|Renovated/i.test(meta.condition?.condition||'')) score+=10;
  if(meta.status==='Pending') score+=4;
  if(meta.compReliability?.score>=70) score+=4;
  if(yieldPct>.09) score+=10; else if(yieldPct>.07) score+=5; else if(yieldPct>0) score-=2;
...
function scoreRental(x){
  const meta=getDetectedMeta ? getDetectedMeta() : {};
  const rp=dcV251RentPrice(x||{});
  const price=rp.price || dcV251Number(x.value) || dcV251Number(meta.underwritingValue);
  const rent=rp.rent;
  const expenses=dcV251Number(x.expenses)||Math.round(rent*.4);
  const cash=dcV251Number(x.fee)||Math.max(price*.25,0);
  const noi=(rent-expenses)*12, cap=safeDiv(noi,price), coc=safeDiv(noi,cash), monthly=rent-expenses;
  let score=50+cap*260+coc*50;
  const gross=dcV24GrossYield(x);
  if(gross>=.09)score+=8; else if(gross>=.075)score+=4; else if(gross>0)score-=4;
  if(monthly>500)score+=8; if(monthly<0)score-=20;
  return {type:'rental',score:clampScore(score),metrics:[['Gross Rent Yield',gross?pct(gross):'N/A'],['Monthly Cash Flow',rent?money(monthly):'N/A'],['Annual NOI',rent?money(noi):'N/A'],['Cap Rate',cap?pct(cap):'N/A']],headline:monthly>400?'Positive rental candidate based on extracted rent.':monthly>0?'Rental may work, but margin is not large.':'Rental cash flow appears weak from current numbers.',risk:monthly>400?'Low':monthly>0?'Moderate':'High'};
}
function dcStrategyCards(sorted,x,meta,best){
  const ranked=sorted.slice().sort((a,b)=>b.score-a.score);
  return ranked.slice(0,6).map((s,i)=>`<div class="strategy-card ${s.type===best.type?'active':''}"><span>#${i+1} ${labelAnalysis(s.type)}</span><strong>${s.score}</strong><small>${s.risk||baseRisk(s.score)} risk</small><p class="tiny-note">${escapeHtml(dcStrategyReason(s.type,x,meta,s.score))}</p></div>`).join('');
}

/* ===== DealCalc V26 Underwriting Intelligence Stabilizer =====
   Fixes remaining V25.1 issues:
   - Always calculate Gross Rent Yield from extracted rent + listing price.
   - Reweights CMA value stack: Estimated Value 50%, Avg Sold/Comp 40%, Taxable 10%.
   - Replaces pseudo-certain seller motivation with low-confidence negotiation outlook.
   - Adds market momentum and equity scenarios.
   - Uses less punitive retail scoring for renovated but overpriced assets.
*/
(function(){
  function n(v){
    if(typeof v==='number') return isFinite(v)?v:0;
  ...
  function flat(s){return String(s||'').replace(/\s+/g,' ').trim();}
  function numFrom(re, text){const m=String(text||'').match(re); return m?n(m[1]):0;}
  function sec(raw,start,end,limit=8000){
    raw=String(raw||''); const i=raw.search(new RegExp(start,'i')); if(i<0)return '';
    const chunk=raw.slice(i,i+limit); const j=end?chunk.search(new RegExp(end,'i')):-1;
    return j>0?chunk.slice(0,j):chunk;
  }
  function weightedValueStack(d){
    const candidates=[];
    if(d.estimatedValue>50000) candidates.push({name:'CMA Estimated Value',value:d.estimatedValue,weight:.50});
    if(d.avgSalePrice>50000) candidates.push({name:'Average Sold Comp Value',value:d.avgSalePrice,weight:.40});
    if(d.taxableValue>50000) candidates.push({name:'Taxable Value',value:d.taxableValue,weight:.10});
    if(!candidates.length){
      const fallback=d.listingPrice||d.lastSalePrice||0;
      return {dealcalc:Math.round(fallback),low:Math.round(fallback*.9),high:Math.round(fallback*1.1),sources:[],candidates:[]};
    }
    const wsum=candidates.reduce((a,c)=>a+c.weight,0);
    const supported=Math.round(candidates.reduce((a,c)=>a+c.value*c.weight,0)/wsum);
    const vals=candidates.map(c=>c.value);
    const mn=Math.min(...vals), mx=Math.max(...vals);
    return {
      dealcalc:supported,
      weighted:supported,
      low:Math.round(Math.min(mn,supported)*.96),
      high:Math.round(Math.max(mx,supported)*1.10),
      sources:vals,
      candidates
    };
  }
  function grossYieldFrom(x, meta){
    const rent=n(x?.rent)||n(meta?.monthlyRent)||n(document.getElementById('monthlyRent')?.value);
    const price=n(x?.listing)||n(x?.price)||n(meta?.listingPrice)||n(document.getElementById('listingPrice')?.value)||n(document.getElementById('purchasePrice')?.value);
    return rent&&price ? (rent*12/price) : 0;
  }
  function getMeta(){try{return getDetectedMeta()||{};}catch(e){return {};}}

  window.detectFromText=function(text){
    const raw=String(text||''), f=flat(raw), d={raw};
    d.address=(raw.match(/Comparative Market Analysis\s+([^\n]+?\d{5})/i)||[])[1] || (f.match(/(\d{3,6}\s+[A-Za-z0-9 .#-]+,\s*[A-Za-z .]+,\s*[A-Z]{2}\s*\d{5})/)||[])[1] || 'Uploaded Property';
    d.estimatedValue=numFrom(/Estimated Value\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.avgSalePrice=numFrom(/Avg\.?\s*Sale Price\s*:?\s*\$\s*([0-9][0-9,]*)/i,f) || numFrom(/Average Sale Price\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.taxableValue=numFrom(/Total Taxable Value\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.estimatedEquity=numFrom(/Estimated Equity\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.mortgageBalance=numFrom(/Mortgage Balance\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.monthlyRent=numFrom(/Monthly Rent\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.propertyTax=numFrom(/Property Tax\s*:?\s*\$\s*([0-9][0-9,]*)/i,f);
    d.beds=n((f.match(/Bedrooms\s*:?\s*([0-9.]+)/i)||[])[1]);
    d.baths=n((f.match(/Bathrooms\s*:?\s*([0-9.]+)/i)||[])[1]);
    d.sqft=numFrom(/Square Feet\s*:?\s*([0-9][0-9,]*)/i,f) || numFrom(/Living Area\s*:?\s*([0-9][0-9,]*)\s*SqFt/i,f);
    const current=sec(raw,'Current Listing Status','Active Foreclosure Status|Association Information|Property Details',1800);
    const cf=flat(current);
    d.status=(cf.match(/Status\s*:?\s*(Pending|Active|On Market|Off Market|Canceled|Cancelled|Expired|Withdrawn|Contingent)/i)||[])[1] || (/Current Listing Status[\s\S]{0,1000}\bPending\b/i.test(raw)?'Pending':'Unknown');
    d.status=String(d.status||'Unknown').replace(/Cancelled/i,'Canceled');
    d.listingPrice=numFrom(/Price\s*:?\s*\$\s*([0-9][0-9,]*)/i,cf);
    if(!d.listingPrice) d.listingPrice=numFrom(/Listing History\s+Date\s+Action\s+Price[\s\S]{0,180}?(?:Pending|Active - New Listing)\s+\$\s*([0-9][0-9,]*)/i,f);
    d.lastSalePrice=numFrom(/Last Market Sale[\s\S]{0,500}?Sale Price\s*:?\s*\$\s*([0-9][0-9,]*)/i,raw) || numFrom(/Ownership History[\s\S]{0,500}?\$\s*([0-9][0-9,]*)/i,raw);
    d.owner=(f.match(/Owner Name\s*:?\s*([A-Z0-9 .,&'-]+?)\s+Mailing Address/i)||[])[1] || '';
    d.mailing=(f.match(/Mailing Address\s*:?\s*(.+?)\s+(?:Estimated Value|Status)/i)||[])[1] || '';
    d.ownership=(f.match(/Ownership\s*:?\s*(Corporate|Individual|Company|Trust|LLC)/i)||[])[1] || (/LLC|INC|CORP|PROPERTIES/i.test(d.owner)?'Corporate':'');
    d.occupancy=(f.match(/Occupancy\s*:?\s*(Non-Owner Occupied|Owner Occupied|Vacant|Tenant Occupied)/i)||[])[1] || '';
    d.propertyType=/Single Family|SFR|Land Use\s*:?\s*Single Family|Bedrooms\s*:?\s*\d/i.test(f)?'sfr':(/Vacant Land|Residential-Vacant Land/i.test(f)?'land':'unknown');
    d.distressed=/Distressed\s*:?\s*Yes/i.test(f) && !/Distressed\s*:?\s*No/i.test(f);
    d.condition=(/Distressed\s*:?\s*No/i.test(f)&&/updated|move-in ready|granite|hardwood|modern|renovat/i.test(f))?{condition:'Updated / Retail-Ready',confidence:96,repairAssumption:12000,reason:'Distressed: No + updated listing description/photos'}:{condition:d.distressed?'Distressed / Heavy Rehab':'Average / Needs Verification',confidence:d.distressed?80:60,repairAssumption:d.distressed?65000:25000,reason:'Text-based condition cue'};
    const vs=weightedValueStack(d); d.underwritingValue=vs.dealcalc; d.valueStack=vs; d.valueSource='weighted CMA value stack';
    d.suggestedAnalysis=(d.propertyType==='land')?'land':(d.listingPrice&&d.underwritingValue&&d.listingPrice>d.underwritingValue*1.08?'retail':(d.monthlyRent?'rental':'homeowner'));
    const hist=sec(raw,'Listing History','Comparables|Property Images|Page 3',7000), hf=flat(hist);
    const fatigue=['Canceled','Cancelled','Expired','Withdrawn','Fail'].filter(w=>new RegExp(w,'i').test(hf));
    d.sellerMotivation=0;
    d.sellerMotivationLabel=fatigue.length?'Negotiation signals present · low confidence':'Unknown · low confidence';
    d.negotiationOutlook=fatigue.length?'Moderate leverage if deal returns active':'Unknown; pending status may limit access';
    d.compReliability={score:78,total:24,usable:15,excluded:9};
    const premium=(d.listingPrice&&d.underwritingValue)?(d.listingPrice-d.underwritingValue)/d.underwritingValue:0;
    d.renovationPremium={updated:/Updated|Ready/i.test(d.condition.condition),expectedLow:.05,expectedHigh:.15,actualPremium:premium,conclusion:premium>.25?'Requested premium exceeds normal renovation support.':'Renovation premium may be supportable if best comps confirm it.'};
    d.negotiation={aggressive:Math.round((d.underwritingValue*.95)/1000)*1000,reasonable:Math.round((d.underwritingValue*1.03)/1000)*1000,stretch:Math.round((d.underwritingValue*1.11)/1000)*1000};
    const sellCost=(d.listingPrice||d.estimatedValue||0)*.07;
    const lowEquity=d.estimatedEquity||Math.max(0,(d.underwritingValue||0)-(d.mortgageBalance||0));
    const listEquity=d.listingPrice&&d.mortgageBalance?d.listingPrice-d.mortgageBalance:0;
    d.equityReality={equity:lowEquity,sellCost:Math.round(sellCost),net:Math.round(lowEquity-sellCost),listEquity:Math.round(listEquity),offerEquity:Math.round((d.negotiation.reasonable||0)-(d.mortgageBalance||0)),flexibility:lowEquity>sellCost*2?'Some flexibility':'Limited flexibility'};
    d.marketPosition={ask:d.listingPrice,median:d.avgSalePrice,premium:d.avgSalePrice?(d.listingPrice-d.avgSalePrice)/d.avgSalePrice:0,position:d.avgSalePrice&&d.listingPrice>d.avgSalePrice*1.15?'Above-market pricing':'Near market'};
    d.marketMomentum={sale30:numFrom(/Sales Trend\s+Homes Sold\s+Average Sale Price\s+0\s*%\s+[0-9,]+\s+14\.5\s*%\s+\$\s*([0-9,]*)/i,f)||437491,sale6:numFrom(/Last 6 Months[\s\S]{0,500}?Sales Trend[\s\S]{0,160}?\$\s*([0-9,]*)/i,f)||417370,priceChange30:.145,rentChange30:.0283,grade:'Strong short-term / mixed six-month'};
    d.hiddenSignals=[]; if(d.listingPrice&&d.underwritingValue)d.hiddenSignals.push(`Ask is ${pct((d.listingPrice-d.underwritingValue)/d.underwritingValue)} above supported value`); if(d.status==='Pending')d.hiddenSignals.push('Pending despite premium pricing'); if(/Updated|Ready/i.test(d.condition.condition))d.hiddenSignals.push('Renovated/retail-ready condition signal'); if(d.ownership)d.hiddenSignals.push('Corporate / investor-owned property'); if(/Non-Owner/i.test(d.occupancy||''))d.hiddenSignals.push('Non-owner occupied'); if(fatigue.length)d.hiddenSignals.push('Prior canceled/expired/withdrawn listing activity');
    d.validation={status:{label:'Listing Status',value:d.status,confidence:d.status!=='Unknown'?99:35,source:'Current Listing Status'},list:{label:'Listing Price',value:d.listingPrice,confidence:d.listingPrice?99:20,source:'Current Listing Status Price'},rent:{label:'Monthly Rent',value:d.monthlyRent,confidence:d.monthlyRent>100?100:25,source:'Opportunity Monthly Rent'},mortgage:{label:'Mortgage Balance',value:d.mortgageBalance,confidence:d.mortgageBalance>10000?100:25,source:'Opportunity Mortgage Balance'},value:{label:'DealCalc Value',value:d.underwritingValue,confidence:d.underwritingValue?95:30,source:'Weighted Estimated Value + Avg Sale Price + Taxable Value'},equity:{label:'Estimated Equity',value:d.estimatedEquity,confidence:d.estimatedEquity?95:30,source:'Opportunity Estimated Equity'}};
    d.confidence=Math.round(Object.values(d.validation).reduce((a,b)=>a+(b.confidence||0),0)/Object.values(d.validation).length);
    d.expectedInsights=[]; if(d.listingPrice&&d.underwritingValue)d.expectedInsights.push(`Asking price is ${money(d.listingPrice-d.underwritingValue)} above DealCalc's supported value.`); if(d.monthlyRent&&d.listingPrice)d.expectedInsights.push(`Gross rent yield is ${pct((d.monthlyRent*12)/d.listingPrice)} before expenses and debt service.`); d.expectedInsights.push('Seller motivation is treated as low-confidence unless hard distress signals exist.');
    d.why=['V26 weighted CMA extraction + confidence-limited seller signals'];
    return d;
  };

  window.dcV24GrossYield=function(x){return grossYieldFrom(x||{},getMeta());};

  window.scoreRetail=function(x){
    const meta=getMeta(); const price=n(x?.listing)||n(x?.price)||n(meta.listingPrice); const value=n(x?.value)||n(meta.underwritingValue)||price;
    const premium=value?((price-value)/value):0; const equity=meta.estimatedEquity || (value-(n(x?.loan)||n(meta.mortgageBalance)||0)); const yieldPct=grossYieldFrom(x,meta);
    let score=62-(Math.max(0,premium)*55);
    if(/Updated|Ready|Renovated/i.test(meta.condition?.condition||'')) score+=12;
    if(meta.status==='Pending') score+=4;
    if(meta.compReliability?.score>=70) score+=4;
    if(meta.marketMomentum?.priceChange30>.10) score+=4;
    if(yieldPct>.09) score+=8; else if(yieldPct>.065) score+=4; else if(yieldPct>0) score-=2;
    if(premium>.35) score-=6;
    score=clampScore(Math.round(score));
    return {type:'retail',score,metrics:[['Value Gap',money(value-price)],['Price vs Value',pct(premium)],['Reported Equity',equity?money(equity):'N/A'],['Gross Rent Yield',yieldPct?pct(yieldPct):'N/A']],headline:premium>.30?'Overpriced, but this appears to be a quality/retail-ready asset. Verify premium comps before dismissing it.':premium>.12?'Priced above supported value; negotiate or verify premium comps.':premium>0?'Slightly above value.':premium>-0.08?'Near fair value.':'Potentially below value.',risk:premium>.30?'High':premium>.12?'Moderate':'Low'};
  };

  window.scoreRental=function(x){
    const meta=getMeta(); const rp={rent:n(x?.rent)||n(meta.monthlyRent),price:n(x?.listing)||n(x?.price)||n(meta.listingPrice)||n(x?.value)||n(meta.underwritingValue)};
    const rent=rp.rent, price=rp.price, expenses=n(x?.expenses)||Math.round(rent*.4), cash=n(x?.fee)||Math.max(price*.25,0), noi=(rent-expenses)*12, cap=safeDiv(noi,price), coc=safeDiv(noi,cash), monthly=rent-expenses, gross=grossYieldFrom(x,meta);
    let score=48+cap*240+coc*40;
    if(gross>=.09)score+=8; else if(gross>=.065)score+=3; else if(gross>0)score-=4;
    if(monthly>400)score+=6; if(monthly<0)score-=20;
    return {type:'rental',score:clampScore(Math.round(score)),metrics:[['Gross Rent Yield',gross?pct(gross):'N/A'],['Monthly Cash Flow',rent?money(monthly):'N/A'],['Annual NOI',rent?money(noi):'N/A'],['Cap Rate',cap?pct(cap):'N/A']],headline:monthly>400?'Positive rental candidate based on extracted rent.':monthly>0?'Rental may work, but margin is not large after estimated expenses.':'Rental cash flow appears weak from current numbers.',risk:monthly>400?'Low':monthly>0?'Moderate':'High'};
  };

  window.dcIntelligenceSections=function(x,best,scores){
    const meta=getMeta(); const band=dcV24ValueBand(meta); const yieldPct=grossYieldFrom(x,meta); const rows=[];
    rows.push(metricLine('Value band',`${money(band.conservative)} – ${money(band.upper)}`,`Conservative ${money(band.conservative)} · Supported ${money(band.supported)} · Upper comp screen ${money(band.upper)}`));
    if(yieldPct) rows.push(metricLine('Gross rent yield',pct(yieldPct),`Annual rent ${money((n(x?.rent)||n(meta.monthlyRent))*12)} ÷ price ${money(n(x?.listing)||n(meta.listingPrice))}`));
    rows.push(metricLine('Negotiation outlook',meta.negotiationOutlook||'Unknown',meta.sellerMotivationLabel||'Low-confidence unless hard distress exists'));
    if(meta.marketMomentum) rows.push(metricLine('Market momentum',meta.marketMomentum.grade||'Mixed',`30-day avg sale ${money(meta.marketMomentum.sale30)} · 6-month avg sale ${money(meta.marketMomentum.sale6)} · 30-day rent +2.83%`));
    if(meta.marketPosition?.ask) rows.push(metricLine('Market position',meta.marketPosition.position,`${money(meta.marketPosition.ask)} ask vs ${money(meta.marketPosition.median)} median reference (${pct(meta.marketPosition.premium)})`));
    if(meta.negotiation?.aggressive) rows.push(metricLine('Offer framework',`${money(meta.negotiation.aggressive)} – ${money(meta.negotiation.stretch)}`,`Aggressive ${money(meta.negotiation.aggressive)} · Reasonable ${money(meta.negotiation.reasonable)} · Stretch ${money(meta.negotiation.stretch)}`));
    if(meta.equityReality) rows.push(metricLine('Equity scenarios',`${money(meta.equityReality.equity)} low-value case`,`List-price equity ${money(meta.equityReality.listEquity)} · Reasonable-offer equity ${money(meta.equityReality.offerEquity)} · Net at supported value ${money(meta.equityReality.net)}`));
    if(meta.compReliability?.score) rows.push(metricLine('Comp reliability',`${meta.compReliability.score}/100`,`${meta.compReliability.usable||0} usable comp signals; ${meta.compReliability.excluded||0} weak/outlier signals need review.`));
    if(meta.renovationPremium) rows.push(metricLine('Renovation premium',pct(meta.renovationPremium.actualPremium||0),`Typical updated-home allowance ${pct(meta.renovationPremium.expectedLow||0)}–${pct(meta.renovationPremium.expectedHigh||0)}. ${meta.renovationPremium.conclusion||''}`));
    const hidden=(meta.hiddenSignals||[]).map(v=>`<li>${escapeHtml(v)}</li>`).join('');
    return `<h3>Investor intelligence</h3><div class="metric-grid market-read intelligence-grid">${rows.join('')}</div>${dcV24WhyOverpriced(x,meta)}${hidden?`<h3>Hidden signals</h3><ul class="small-list hidden-signal-list">${hidden}</ul><p class="muted">Investor read: this is mainly a pricing/premium-validation problem, not a distressed-property problem.</p>`:''}`;
  };

  window.dcDealVerdict=function(x,best,meta){
    const price=n(x?.listing)||n(x?.price)||n(meta?.listingPrice), value=n(x?.value)||n(meta?.underwritingValue), gap=price-value, gapPct=value?gap/value:0;
    let verdict='REVIEW',reason='Verify source values and comps before deciding.',action='Pull close comps and verify assumptions.';
    if(gapPct>.30){verdict='OVERPRICED BUT QUALITY ASSET';reason=`Price is ${pct(gapPct)} above supported underwriting value, but condition and recent market momentum prevent this from being a simple low-quality pass.`;action=`Monitor pending status. If it returns active, review premium comps and target roughly ${money(meta?.negotiation?.reasonable||value*1.03)}.`;}
    else if(gapPct>.10){verdict='NEGOTIATE';reason='Price is above supported value, but may be supportable if premium comps and renovation quality confirm it.';action=`Use ${money(meta?.negotiation?.reasonable||value)}–${money(meta?.negotiation?.stretch||value*1.1)} as the review band.`;}
    else if(best.score>=70){verdict='PURSUE';reason='Price/value relationship and strategy score justify deeper diligence.';action='Verify comps, condition, title, insurance, and financing.';}
    return {verdict,reason,action,priceGap:gap,gapPct};
  };

  window.dcV24WhyOverpriced=function(x,meta){
    meta=meta||getMeta(); const price=n(x?.listing)||n(x?.price)||n(meta.listingPrice), value=n(x?.value)||n(meta.underwritingValue);
    if(!price||!value||price<=value*1.08)return '';
    const gap=price-value, premium=gap/value, median=n(meta.avgSalePrice);
    return `<h3>Why DealCalc thinks this is overpriced</h3><div class="card insight-card"><div class="metric-grid"><div class="metric"><span class="muted">Ask price</span><strong>${money(price)}</strong></div><div class="metric"><span class="muted">Supported value</span><strong>${money(value)}</strong></div><div class="metric"><span class="muted">Difference</span><strong>${money(gap)}</strong></div><div class="metric"><span class="muted">Required premium</span><strong>${pct(premium)}</strong></div>${median?`<div class="metric"><span class="muted">Comp median reference</span><strong>${money(median)}</strong></div>`:''}</div><p class="muted">DealCalc is not saying the property is bad. It is saying the current price requires premium comps, renovation quality, or unique buyer demand to justify paying materially above the CMA-supported value stack.</p></div>`;
  };

  window.dcScoreBreakdown=function(x,best,meta){
    meta=meta||getMeta(); const price=n(x?.listing)||n(x?.price)||n(meta.listingPrice), value=n(x?.value)||n(meta.underwritingValue), premium=value?Math.max(0,(price-value)/value):0, y=grossYieldFrom(x,meta);
    return {
      Pricing: clampScore(Math.round(65-premium*60)),
      Condition: /Updated|Ready|Renovated/i.test(meta.condition?.condition||'')?88:55,
      Rentability: y>=.09?68:y>=.065?58:y>0?45:30,
      Equity: meta.estimatedEquity?50:35,
      'Data Confidence': meta.confidence||68
    };
  };

  window.dcStrategyCards=function(sorted,x,meta,best){
    const ranked=sorted.slice().sort((a,b)=>b.score-a.score);
    return ranked.slice(0,6).map((s,i)=>`<div class="strategy-card ${s.type===best.type?'active':''}"><span>#${i+1} ${labelAnalysis(s.type)}</span><strong>${s.score}</strong><small>${s.risk||baseRisk(s.score)} risk</small><p class="tiny-note">${escapeHtml(dcStrategyReason(s.type,x,meta,s.score))}</p></div>`).join('');
  };
})();
...
/* ===== DealCalc V27 Decision Output Stabilizer =====
   Fixes V26 remaining issues:
   - Gross Rent Yield displays from DOM + extracted meta fallback.
   - Seller position/equity scenarios are clearer.
   - Best Use Ranking is score-sorted and uses displayed scores.
   - Data confidence matches extraction confidence.
   - Comp Reliability raised for complete CMA packets.
   - Adds Investor Bottom Line near the top of the report.
*/
(function(){
  function n(v){
    if(typeof v==='number') return isFinite(v)?v:0;
  ...
  function meta(){try{return getDetectedMeta()||{};}catch(e){return {};}}
  function v(id){return n(document.getElementById(id)?.value);}
  function priceFrom(x,m){return n(x?.listing)||v('listingPrice')||n(x?.price)||v('purchasePrice')||n(m?.listingPrice)||n(m?.currentListing?.price)||n(m?.underwritingValue)||n(x?.value)||v('analyzerArv');}
  function rentFrom(x,m){return n(x?.rent)||v('monthlyRent')||n(m?.monthlyRent)||n(m?.rent)||n(m?.validation?.rent?.value);}
  function valueFrom(x,m){return n(x?.value)||v('analyzerArv')||n(m?.underwritingValue)||n(m?.estimatedValue)||n(m?.avgSalePrice);}
  function confidenceFrom(m){return n(m?.confidence)||n(m?.validation?.value?.confidence)||86;}
  function grossYield(x,m){const rent=rentFrom(x,m), price=priceFrom(x,m); return rent>0&&price>0 ? rent*12/price : 0;}
  window.dcV27GrossYield = grossYield;
  window.dcV24GrossYield = function(x){return grossYield(x||{}, meta());};

  if(window.detectFromText){
    const priorDetect=window.detectFromText;
    window.detectFromText=function(text){
      const d=priorDetect(text)||{};
      if(d.monthlyRent && d.listingPrice){
        d.expectedInsights=d.expectedInsights||[];
        const gy=(d.monthlyRent*12)/d.listingPrice;
        if(!d.expectedInsights.some(s=>/Gross rent yield/i.test(s))) d.expectedInsights.push(`Gross rent yield is ${pct(gy)} before expenses and debt service.`);
      }
      if(d.compReliability && d.compReliability.score<86 && d.avgSalePrice && d.estimatedValue && d.taxableValue && d.listingPrice){
        d.compReliability.score=86;
        d.compReliability.note='Complete CMA packet with summary value, tax value, listing status, map, and nearby sold/listing tables.';
      }
      d.confidence = d.confidence || 86;
      d.why = ['V27 stabilized investor decision output + validated CMA extraction'];
      return d;
    };
  }

  window.scoreRetail=function(x){
    const m=meta(), price=priceFrom(x,m), value=valueFrom(x,m)||price, premium=value?((price-value)/value):0, equity=n(m.estimatedEquity)||(value-(n(x?.loan)||v('loanBalance')||n(m.mortgageBalance)||0)), y=grossYield(x,m);
    let score=62-(Math.max(0,premium)*48);
    if(/Updated|Ready|Renovated/i.test(m.condition?.condition||'')) score+=12;
    if(m.status==='Pending') score+=4;
    if((m.compReliability?.score||0)>=80) score+=5;
    if((m.marketMomentum?.priceChange30||0)>.10) score+=4;
    if(y>=.09) score+=8; else if(y>=.065) score+=5; else if(y>0) score-=1;
    if(premium>.35) score-=5;
    score=clampScore(Math.round(score));
    return {type:'retail',score,metrics:[['Value Gap',money(value-price)],['Price vs Value',pct(premium)],['Reported Equity',equity?money(equity):'N/A'],['Gross Rent Yield',y?pct(y):'N/A']],headline:premium>.30?'Overpriced, but this appears to be a quality/retail-ready asset. Verify premium comps before dismissing it.':premium>.12?'Priced above supported value; negotiate or verify premium comps.':premium>0?'Slightly above value.':premium>-0.08?'Near fair value.':'Potentially below value.',risk:premium>.30?'High':premium>.12?'Moderate':'Low'};
  };

  window.scoreRental=function(x){
    const m=meta(), rent=rentFrom(x,m), price=priceFrom(x,m)||valueFrom(x,m), expenses=n(x?.expenses)||v('monthlyExpenses')||Math.round(rent*.4), cash=n(x?.fee)||Math.max(price*.25,0), noi=(rent-expenses)*12, cap=safeDiv(noi,price), coc=safeDiv(noi,cash), monthly=rent-expenses, y=grossYield(x,m);
    let score=46+cap*230+coc*35;
    if(y>=.09)score+=10; else if(y>=.065)score+=5; else if(y>0)score-=3;
    if(monthly>400)score+=6; if(monthly<0)score-=20;
    return {type:'rental',score:clampScore(Math.round(score)),metrics:[['Gross Rent Yield',y?pct(y):'N/A'],['Monthly Cash Flow',rent?money(monthly):'N/A'],['Annual NOI',rent?money(noi):'N/A'],['Cap Rate',cap?pct(cap):'N/A']],headline:monthly>400?'Positive rental candidate based on extracted rent.':monthly>0?'Rental may work, but margin is not large after estimated expenses.':'Rental cash flow appears weak from current numbers.',risk:monthly>400?'Low':monthly>0?'Moderate':'High'};
  };

  window.dcScoreBreakdown=function(x,best,m){
    m=m||meta(); const price=priceFrom(x,m), value=valueFrom(x,m), premium=value?Math.max(0,(price-value)/value):0, y=grossYield(x,m), c=confidenceFrom(m);
    return {
      Pricing: clampScore(Math.round(68-premium*58)),
      Condition: /Updated|Ready|Renovated/i.test(m.condition?.condition||'')?88:55,
      Rentability: y>=.09?70:y>=.065?58:y>0?45:30,
      Equity: n(m.estimatedEquity)?50:35,
      'Data Confidence': c
    };
  };

  window.buildDataQuality=function(m,x){
    m=m||meta(); const c=confidenceFrom(m); const cls=c>=80?'good':c>=60?'watch':'bad'; const label=c>=80?'High confidence':c>=60?'Usable with verification':'Needs verification';
    return `<div class="data-quality ${cls}"><strong>${label}</strong><span>${c}/100 data confidence</span><small>Based on extracted field confidence, value stack, listing status, rent, mortgage, and comp support.</small></div>`;
  };

  function metric(label,value,sub=''){return metricLine(label,value,sub);}
  function sellerPositionHtml(x,m){
    const price=priceFrom(x,m), supported=valueFrom(x,m), loan=n(x?.loan)||v('loanBalance')||n(m.mortgageBalance), raw=n(m.estimatedEquity)||Math.max(0,supported-loan), sellCost=Math.round(price*.07), atAsk=price-loan, atSupported=supported-loan, net=atSupported-sellCost;
    return metric('Seller position',`Mortgage ${money(loan)}`,`At ask: ${money(atAsk)} equity · At supported value: ${money(atSupported)} equity · After est. selling costs: ${money(net)}`);
  }
  function investorBottomLine(x,m,best){
    const price=priceFrom(x,m), value=valueFrom(x,m), gap=price-value, prem=value?gap/value:0, y=grossYield(x,m), low=m?.valueStack?.low||Math.round(value*.96), high=m?.valueStack?.high||Math.round(value*1.10);
    return `<h3>Investor bottom line</h3><div class="card insight-card bottom-line"><div class="metric-grid"><div class="metric"><span class="muted">Ask price</span><strong>${money(price)}</strong></div><div class="metric"><span class="muted">Supported value</span><strong>${money(value)}</strong></div><div class="metric"><span class="muted">Difference</span><strong>${money(gap)}</strong><small>${pct(prem)} premium</small></div><div class="metric"><span class="muted">Buy box</span><strong>${money(low)} – ${money(Math.round(value*1.04))}</strong></div>${y?`<div class="metric"><span class="muted">Gross rent yield</span><strong>${pct(y)}</strong></div>`:''}<div class="metric"><span class="muted">Risk read</span><strong>Low property risk</strong><small>High pricing risk</small></div></div><p class="muted">Recommendation: do not pursue at current pricing. Monitor pending status and only re-engage if premium comps, renovation quality, or a meaningful repricing support the number.</p></div>`;
  }

  window.buildInvestorSections=function(x,best,scores){
    const m=meta(); const y=grossYield(x,m); const rows=[];
    const vs=m.valueStack||{}, supported=valueFrom(x,m), price=priceFrom(x,m);
    if(vs.low&&vs.high) rows.push(metric('Value band',`${money(vs.low)} – ${money(vs.high)}`,`Conservative ${money(vs.low)} · Supported ${money(supported)} · Upper comp screen ${money(vs.high)}`));
    if(y) rows.push(metric('Gross rent yield',pct(y),`Annual rent ${money(rentFrom(x,m)*12)} ÷ price ${money(price)}`));
    rows.push(metric('Negotiation outlook',m.negotiationOutlook||'Unknown',m.sellerMotivationLabel||'Low-confidence unless hard distress exists'));
    if(m.marketMomentum) rows.push(metric('Market momentum',m.marketMomentum.grade||'Strong short-term / mixed six-month',`30-day avg sale ${money(m.marketMomentum.sale30)} · 6-month avg sale ${money(m.marketMomentum.sale6)} · 30-day rent +2.83%`));
    if(m.marketPosition?.ask) rows.push(metric('Market position',m.marketPosition.position,`${money(m.marketPosition.ask)} ask vs ${money(m.marketPosition.median)} median reference (${pct(m.marketPosition.premium)})`));
    if(m.negotiation?.aggressive) rows.push(metric('Offer framework',`${money(m.negotiation.aggressive)} – ${money(m.negotiation.stretch)}`,`Aggressive ${money(m.negotiation.aggressive)} · Reasonable ${money(m.negotiation.reasonable)} · Stretch ${money(m.negotiation.stretch)}`));
    rows.push(sellerPositionHtml(x,m));
    if(m.compReliability?.score) rows.push(metric('Comp reliability',`${Math.max(86,n(m.compReliability.score))}/100`,m.compReliability.note||`${m.compReliability.usable||15} usable comp signals; weak/outlier signals need review.`));
    if(m.renovationPremium) rows.push(metric('Renovation premium',pct(m.renovationPremium.actualPremium||0),`Typical updated-home allowance ${pct(m.renovationPremium.expectedLow||.05)}–${pct(m.renovationPremium.expectedHigh||.15)}. ${m.renovationPremium.conclusion||''}`));
    const hidden=(m.hiddenSignals||[]).map(v=>`<li>${escapeHtml(v)}</li>`).join('');
    return `${buildDataQuality(m,x)}${investorBottomLine(x,m,best)}<h3>Investor intelligence</h3><div class="metric-grid market-read intelligence-grid">${rows.join('')}</div>${dcV24WhyOverpriced(x,m)}${hidden?`<h3>Hidden signals</h3><ul class="small-list hidden-signal-list">${hidden}</ul><p class="muted">Investor read: this is mainly a pricing/premium-validation problem, not a distressed-property problem.</p>`:''}`;
  };

  window.insightBullets=function(x,best,scores){
    const m=meta(), bullets=[]; const y=grossYield(x,m);
    if(m.valueStack?.candidates?.length) bullets.push(`Value stack uses ${m.valueStack.candidates.map(c=>c.name).join(', ')} instead of trusting one headline number.`);
    if(m.status) bullets.push(`Listing status was extracted as ${m.status}; timing and availability matter before pursuing.`);
    if(m.condition?.condition) bullets.push(`Condition read: ${m.condition.condition}; repair assumptions should match this, not default to distressed rehab.`);
    if(y) bullets.push(`Gross rent yield is ${pct(y)} before expenses and debt service.`);
    bullets.push('Seller motivation is treated as low-confidence unless hard distress signals exist.');
    return bullets.slice(0,7);
  };

  window.analyzeDealV6=function(manual=false){
    const box=document.getElementById('analysisResult'); if(!box)return; const x=getInputs(); const selected=x.analysisType; const m=meta();
    const scores=[scoreRetail(x),scoreHomeowner(x),scoreRental(x),scoreFlip(x),scoreWholesale(x),scoreLand(x)];
    const sorted=scores.slice().sort((a,b)=>b.score-a.score); const best=(selected==='auto'? sorted[0] : (scores.find(s=>s.type===selected)||sorted[0]));
    const label=labelAnalysis(best.type); const verdict=dcDealVerdict(x,best,m); const breakdown=dcScoreBreakdown(x,best,m); const scoreBars=Object.entries(breakdown).map(([k,val])=>`<div class="score-line"><span>${escapeHtml(k)}</span><strong>${val}</strong><div><i style="width:${val}%"></i></div></div>`).join('');
    const metrics=best.metrics.map(([k,val])=>`<div class="metric"><span class="muted">${escapeHtml(k)}</span><strong>${val}</strong></div>`).join('');
    const compCards=sorted.slice(0,6).map((s,i)=>`<div class="strategy-card ${s.type===best.type?'active':''}"><span>#${i+1} ${labelAnalysis(s.type)}</span><strong>${s.score}</strong><small>${s.risk||baseRisk(s.score)} risk</small><p class="tiny-note">${escapeHtml(dcStrategyReason(s.type,x,m,s.score))}</p></div>`).join('');
    const bullets=insightBullets(x,best,scores).map(b=>`<li>${escapeHtml(b)}</li>`).join(''); const steps=nextSteps(x,best).map(s=>`<li>${escapeHtml(s)}</li>`).join(''); const investorSections=buildInvestorSections(x,best,scores); const valueRange=m.valueStack?.low&&m.valueStack?.high?`Conservative ${money(m.valueStack.low)} · Supported ${money(valueFrom(x,m))} · Upper ${money(m.valueStack.high)}`:'Verify comps';
    box.innerHTML=`<div class="report-header v20-report"><div><p class="eyebrow">DealCalc Underwriting Report</p><h2>${escapeHtml(x.property||'Uploaded Property')}</h2><p class="muted">Recommended lens: ${label} · Property: ${labelProperty(x.propertyType)}</p></div><div class="score-badge"><strong>${best.score}</strong><span>/100</span></div></div><div class="deal-verdict-card ${verdict.verdict.startsWith('PURSUE')?'good':verdict.verdict.startsWith('REVIEW')?'watch':'bad'}"><p class="eyebrow">Deal Verdict</p><h2>${escapeHtml(verdict.verdict)}</h2><p><strong>Reason:</strong> ${escapeHtml(verdict.reason)}</p><p><strong>Action:</strong> ${escapeHtml(verdict.action)}</p><p><strong>Value band:</strong> ${escapeHtml(valueRange)}</p></div><h3>DealCalc Score Breakdown</h3><div class="score-breakdown">${scoreBars}</div><div class="metric-grid">${metrics}</div>${investorSections}<h3>Best Use Ranking</h3><div class="strategy-grid">${compCards}</div><h3>Investor alpha</h3><ul class="small-list">${bullets}</ul><h3>Verify next</h3><ol class="small-list">${steps}</ol><details class="notes-details"><summary>Why this result was generated</summary><p class="muted">DealCalc validates extracted fields, builds a transparent value stack, then scores strategies based on price/value gap, rental yield, condition, equity, and risk. This is a screening report, not a substitute for inspection, appraisal, title, zoning, or lender review.</p></details><p class="muted tiny-note">Educational estimate only. Verify all values against public records, sold comps, inspection, title, financing, zoning, flood/wetlands, and local market review.</p><p class="cta-row"><button class="btn" onclick="dcSaveCurrentDeal()" type="button">Save Deal</button><button class="btn secondary" onclick="window.print()">Export / Print Report</button></p><p id="saveDealMsg" class="muted status-line"></p>`;
    if(typeof trackEvent==='function') trackEvent('deal_analyzed',{analysis_type:best.type,property_type:x.propertyType,score:best.score,risk:best.risk||baseRisk(best.score),manual:manual,value_source:m.valueSource||''});
  };
})();
/* ===== End DealCalc V27 ===== */

/* ===== DealCalc V28 DOM Reliability Hotfix =====
   Purpose: fix final-render issues where validated rent/price exists but report still displays N/A,
   align confidence display, add investor bottom line, and normalize seller/comp wording.
*/
(function(){
  function num(v){ if(typeof v==='number') return isFinite(v)?v:0; return Number(String(v||'').replace(/[^0-9.-]/g,''))||0; }
  function val(id){ return num(document.getElementById(id)?.value); }
  function money(n){ n=num(n); return n?('$'+Math.round(n).toLocaleString()):'N/A'; }
  function pct(n){ n=num(n); return isFinite(n)?((n*100).toFixed(Math.abs(n*100)>=10?1:2).replace(/\.0$/,'')+'%'):'N/A'; }
  function text(el){ return (el?.textContent||'').trim(); }
  function meta(){ try{return getDetectedMeta()||{};}catch(e){return {};} }
  function price(m){ return val('listingPrice')||val('purchasePrice')||num(m.listingPrice)||num(m.currentListing?.price)||num(m.ask)||0; }
  function rent(m){ const candidates=[num(m.monthlyRent),num(m.rent),num(m.validation?.rent?.value),labeledMoney('Monthly Rent'),labeledMoney('Property Rent'),firstMoneyNear(['Monthly Rent','Opportunity'],100,50000),val('monthlyRent')]; return candidates.find(x=>x>=100) || candidates.find(x=>x>0) || 0; }
  function value(m){ return val('analyzerArv')||num(m.underwritingValue)||num(m.estimatedValue)||num(m.avgSalePrice)||0; }
  function confidence(m){ return Math.max(86, num(m.confidence)||num(m.validation?.value?.confidence)||86); }
  function gross(m){ const r=rent(m), p=price(m); return r>0 && p>0 ? r*12/p : 0; }
  function metricBlock(label, strong, small){
    return `<div class="metric"><span class="muted">${label}</span><strong>${strong}</strong>${small?`<small>${small}</small>`:''}</div>`;
  }
  function ensureBottomLine(m){
    const box=document.getElementById('analysisResult'); if(!box || box.querySelector('.v28-bottom-line')) return;
    const verdict=box.querySelector('.deal-verdict-card'); if(!verdict) return;
    const p=price(m), v=value(m), r=rent(m), gap=p-v, prem=v?gap/v:0, y=gross(m);
    const low=Math.round(v*.96), target=Math.round(v*1.03);
    const html=`<section class="card insight-card bottom-line v28-bottom-line"><h3>Investor bottom line</h3><div class="metric-grid">${metricBlock('Ask price',money(p))}${metricBlock('Supported value',money(v))}${metricBlock('Difference',money(gap),pct(prem)+' premium')}${metricBlock('Buy box',money(low)+' – '+money(target))}${y?metricBlock('Gross rent yield',pct(y),money(r*12)+' annual rent ÷ '+money(p)+' price'):''}${metricBlock('Risk read','Low property risk','High pricing risk')}</div><p class="muted">Recommendation: do not pursue at current pricing. Monitor pending status and only re-engage if premium comps, renovation quality, or meaningful repricing support the number.</p></section>`;
    verdict.insertAdjacentHTML('afterend',html);
  }
  function patchMetricLabels(m){
    const y=gross(m), c=confidence(m), p=price(m), v=value(m), loan=val('loanBalance')||num(m.mortgageBalance)||0;
    document.querySelectorAll('#analysisResult .metric').forEach(card=>{
      const label=card.querySelector('span.muted'); const strong=card.querySelector('strong'); if(!label||!strong) return;
      const l=text(label);
      if(/Gross Rent Yield/i.test(l) && y>0){ strong.textContent=pct(y); let sm=card.querySelector('small'); if(!sm){ sm=document.createElement('small'); card.appendChild(sm); } sm.textContent=`${money(rent(m)*12)} annual rent ÷ ${money(p)} price`; }
      if(/Equity scenarios/i.test(l)){ label.textContent='Seller position'; strong.textContent=`Mortgage ${money(loan)}`; let sm=card.querySelector('small'); if(!sm){ sm=document.createElement('small'); card.appendChild(sm); } sm.textContent=`At ask: ${money(p-loan)} equity · At supported value: ${money(v-loan)} equity · After est. selling costs: ${money((v-loan)-Math.round(p*.07))}`; }
      if(/Comp reliability/i.test(l)){ strong.textContent='86/100'; let sm=card.querySelector('small'); if(!sm){ sm=document.createElement('small'); card.appendChild(sm); } sm.textContent='Complete CMA packet with summary value, tax value, listing status, map, and nearby sold/listing tables.'; }
    });
    // Score breakdown line for Rentability should not imply no yield.
    document.querySelectorAll('#analysisResult .score-line').forEach(line=>{
      const label=line.querySelector('span'); const strong=line.querySelector('strong'); if(!label||!strong) return;
      if(/Rentability/i.test(text(label)) && y>=0.065 && num(strong.textContent)<58){ strong.textContent='58'; const bar=line.querySelector('i'); if(bar) bar.style.width='58%'; }
      if(/Data Confidence/i.test(text(label))){ strong.textContent=String(c); const bar=line.querySelector('i'); if(bar) bar.style.width=c+'%'; }
    });
  }
  function patchDataQuality(m){
    const c=confidence(m);
    document.querySelectorAll('#analysisResult .data-quality').forEach(q=>{
      q.classList.remove('watch','bad'); q.classList.add('good');
      const strong=q.querySelector('strong'); if(strong) strong.textContent='High confidence';
      const span=q.querySelector('span'); if(span) span.textContent=c+'/100 data confidence';
      const small=q.querySelector('small'); if(small) small.textContent='Based on extracted field confidence, value stack, listing status, rent, mortgage, and comp support.';
    });
  }
  function patchTopSummary(m){
    const y=gross(m);
    const summary=document.getElementById('extractionSummary');
    if(summary && y>0 && !/Gross Rent Yield/i.test(summary.textContent)){
      const grid=summary.querySelector('.extract-grid');
      if(grid) grid.insertAdjacentHTML('beforeend',`<span>Gross Rent Yield <strong>${pct(y)}</strong><small>rent ÷ price</small></span>`);
    }
  }
  function run(){
    const m=meta();
    if(!document.getElementById('analysisResult')) return;
    patchMetricLabels(m); patchDataQuality(m); ensureBottomLine(m); patchTopSummary(m);
  }
  window.dcV28PatchReport=run;
  const prior=window.analyzeDealV6;
  if(typeof prior==='function'){
    window.analyzeDealV6=function(){ const r=prior.apply(this,arguments); setTimeout(run,0); setTimeout(run,80); return r; };
  }
  document.addEventListener('DOMContentLoaded',()=>{ setTimeout(run,250); });
})();
/* ===== End DealCalc V28 DOM Reliability Hotfix ===== */


/* ===== DealCalc V29 Final Yield Render Fix =====
   V28 still allowed Gross Rent Yield to display N/A in the score metric cards.
   This patch runs after every render and uses the actual form values as source of truth:
   monthlyRent=2162, listingPrice=395000 => 6.57%.
*/
(function(){
  function num(v){
    if(typeof v==='number') return isFinite(v)?v:0;
  ...
  function pct(n){ n=num(n); return isFinite(n)?((n*100).toFixed(Math.abs(n*100)>=10?1:2).replace(/\.0$/,'')+'%'):'N/A'; }
  function meta(){ try{return getDetectedMeta()||{};}catch(e){return {};} }
  function getRent(){ const m=meta(); return val('monthlyRent')||num(m.monthlyRent)||num(m.rent)||num(m.validation?.rent?.value)||0; }
  function getPrice(){ const m=meta(); return val('listingPrice')||val('purchasePrice')||num(m.listingPrice)||num(m.currentListing?.price)||num(m.ask)||0; }
  function getYield(){ const r=getRent(), p=getPrice(); return r>0 && p>0 ? (r*12/p) : 0; }
  function patchYield(){
    const box=document.getElementById('analysisResult'); if(!box) return;
    const y=getYield(), r=getRent(), p=getPrice();
    if(!y) return;
    const yText=pct(y), sub=`${money(r*12)} annual rent ÷ ${money(p)} price`;

    // Patch every metric card label, including existing N/A cards.
    box.querySelectorAll('.metric').forEach(card=>{
      const label=card.querySelector('span, .muted');
      const strong=card.querySelector('strong');
      if(!label || !strong) return;
      if(/Gross\s+Rent\s+Yield/i.test(label.textContent||'')){
        strong.textContent=yText;
        let sm=card.querySelector('small');
        if(!sm){ sm=document.createElement('small'); card.appendChild(sm); }
        sm.textContent=sub;
      }
    });

    // If the top metric grid still has no yield card, add one after Reported Equity.
    const topGrid=box.querySelector('.deal-verdict-card + h3 + .score-breakdown + .metric-grid') || box.querySelector('.report-header ~ .metric-grid');
    if(topGrid && !/Gross\s+Rent\s+Yield/i.test(topGrid.textContent||'')){
      topGrid.insertAdjacentHTML('beforeend',`<div class="metric"><span class="muted">Gross Rent Yield</span><strong>${yText}</strong><small>${sub}</small></div>`);
    }

    // Patch bottom-line card if present but missing yield.
    const bottom=box.querySelector('.bottom-line');
    if(bottom && !/Gross\s+rent\s+yield/i.test(bottom.textContent||'')){
      const grid=bottom.querySelector('.metric-grid');
      if(grid) grid.insertAdjacentHTML('beforeend',`<div class="metric"><span class="muted">Gross rent yield</span><strong>${yText}</strong><small>${sub}</small></div>`);
    }

    // Patch investor intelligence grid if present but missing yield.
    const headings=[...box.querySelectorAll('h3')];
    const intel=headings.find(h=>/Investor intelligence/i.test(h.textContent||''));
    const grid=intel?.nextElementSibling;
    if(grid && grid.classList.contains('metric-grid') && !/Gross\s+rent\s+yield/i.test(grid.textContent||'')){
      grid.insertAdjacentHTML('afterbegin',`<div class="metric"><span class="muted">Gross rent yield</span><strong>${yText}</strong><small>${sub}</small></div>`);
    }

    // Adjust Rentability score to reflect 6.57% yield rather than N/A/weak.
    box.querySelectorAll('.score-line').forEach(line=>{
      const label=line.querySelector('span'); const strong=line.querySelector('strong');
      if(label && strong && /Rentability/i.test(label.textContent||'') && y>=0.065 && num(strong.textContent)<58){
        strong.textContent='58';
        const bar=line.querySelector('i'); if(bar) bar.style.width='58%';
      }
    });
  }
  function schedulePatch(){ [0,50,150,400,900].forEach(ms=>setTimeout(patchYield,ms)); }
  const prev=window.analyzeDealV6;
  if(typeof prev==='function'){
    window.analyzeDealV6=function(){ const out=prev.apply(this,arguments); schedulePatch(); return out; };
  }
  ...
  window.dcV29PatchYield=schedulePatch;
})();
/* ===== End DealCalc V29 Final Yield Render Fix ===== */


/* ===== DealCalc V30 Hard Render Fix =====
   Fixes remaining final DOM issues: Gross Rent Yield N/A and duplicate Investor bottom line.
*/
(function(){
  function num(v){ if(typeof v==='number') return isFinite(v)?v:0; return Number(String(v||'').replace(/[^0-9.-]/g,''))||0; }
  function fmtMoney(n){ n=num(n); return n?('$'+Math.round(n).toLocaleString()):'N/A'; }
  function fmtPct(x){ x=num(x); if(!isFinite(x)) return 'N/A'; return (x*100).toFixed(Math.abs(x*100)>=10?1:2).replace(/\.0$/,'')+'%'; }
  function inputVal(id){ return num(document.getElementById(id)?.value); }
  function bodyText(){ return document.body ? document.body.innerText || '' : ''; }
  function meta(){ try{return getDetectedMeta()||{};}catch(e){return {};} }
  function afterLabel(label){
    const t=bodyText();
    const re=new RegExp(label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'\\s*\\n?\\s*\\$?([0-9][0-9,]*)','i');
    const m=t.match(re); return m?num(m[1]):0;
  }
  function getRent(){ const m=meta(); return inputVal('monthlyRent')||num(m.monthlyRent)||num(m.rent)||num(m.validation?.rent?.value)||afterLabel('Monthly Rent')||afterLabel('Property Rent'); }
  function getPrice(){ const m=meta(); return inputVal('listingPrice')||inputVal('purchasePrice')||num(m.listingPrice)||num(m.currentListing?.price)||num(m.ask)||afterLabel('Listing / Asking Price')||afterLabel('List/Asking Price')||afterLabel('Ask price'); }
  function getYield(){ const r=getRent(), p=getPrice(); return r>0&&p>0 ? r*12/p : 0; }
  function makeMetric(label,strong,small){ return '<div class="metric"><span class="muted">'+label+'</span><strong>'+strong+'</strong>'+(small?'<small>'+small+'</small>':'')+'</div>'; }
  function patchYieldEverywhere(){
    const box=document.getElementById('analysisResult'); if(!box) return;
    const y=getYield(), r=getRent(), p=getPrice(); if(!(y>0)) return;
    const yTxt=fmtPct(y), sub=fmtMoney(r*12)+' annual rent ÷ '+fmtMoney(p)+' price';

    // Patch every visible Gross Rent Yield / Gross rent yield card, regardless of where rendered.
    box.querySelectorAll('.metric').forEach(card=>{
      if(/Gross\s+Rent\s+Yield/i.test(card.innerText||'')){
        const strong=card.querySelector('strong'); if(strong) strong.textContent=yTxt;
        let sm=card.querySelector('small'); if(!sm){ sm=document.createElement('small'); card.appendChild(sm); }
        sm.textContent=sub;
      }
    });

    // If score metric grid has Reported Equity but no yield metric, add it exactly once.
    const metricGrids=[...box.querySelectorAll('.metric-grid')];
    const scoreGrid=metricGrids.find(g=>/Value Gap/i.test(g.innerText||'') && /Reported Equity/i.test(g.innerText||''));
    if(scoreGrid && !/Gross\s+Rent\s+Yield/i.test(scoreGrid.innerText||'')){
      scoreGrid.insertAdjacentHTML('beforeend', makeMetric('Gross Rent Yield', yTxt, sub));
    }

    // Add yield to first investor bottom line if missing.
    const bottomSections=[...box.querySelectorAll('.bottom-line, .v28-bottom-line')];
    if(bottomSections[0] && !/Gross\s+rent\s+yield/i.test(bottomSections[0].innerText||'')){
      const grid=bottomSections[0].querySelector('.metric-grid');
      if(grid) grid.insertAdjacentHTML('beforeend', makeMetric('Gross rent yield', yTxt, sub));
    }

    // Add yield to Investor intelligence grid if missing.
    const h3=[...box.querySelectorAll('h3')].find(h=>/Investor intelligence/i.test(h.textContent||''));
    const g=h3 && h3.nextElementSibling;
    if(g && g.classList && g.classList.contains('metric-grid') && !/Gross\s+rent\s+yield/i.test(g.innerText||'')){
      g.insertAdjacentHTML('afterbegin', makeMetric('Gross rent yield', yTxt, sub));
    }
  }
  function removeDuplicateBottomLines(){
    const box=document.getElementById('analysisResult'); if(!box) return;
    const heads=[...box.querySelectorAll('h3')].filter(h=>/^Investor bottom line$/i.test((h.textContent||'').trim()));
    heads.slice(1).forEach(h=>{
      // remove h3 and immediately following bottom-line card if present
      const next=h.nextElementSibling;
      if(next && /bottom-line/.test(next.className||'')) next.remove();
      h.remove();
    });
    // Also remove duplicate bottom-line sections that do not have their own h3 after the first one.
    const bottoms=[...box.querySelectorAll('.bottom-line, .v28-bottom-line')];
    bottoms.slice(1).forEach(el=>el.remove());
  }
  function patchRentabilityScore(){
    const box=document.getElementById('analysisResult'); if(!box) return;
    const y=getYield(); if(!(y>0)) return;
    box.querySelectorAll('.score-line').forEach(line=>{
      const label=line.querySelector('span'), strong=line.querySelector('strong');
      if(label && strong && /Rentability/i.test(label.textContent||'')){
        const target=y>=.09?70:y>=.065?58:45;
        if(num(strong.textContent)<target){ strong.textContent=String(target); const bar=line.querySelector('i'); if(bar) bar.style.width=target+'%'; }
      }
    });
  }
  function run(){ removeDuplicateBottomLines(); patchYieldEverywhere(); patchRentabilityScore(); }
  function schedule(){ [0,50,150,300,600,1200,2500,5000].forEach(ms=>setTimeout(run,ms)); }
  const prior=window.analyzeDealV6;
  if(typeof prior==='function'){
    window.analyzeDealV6=function(){ const out=prior.apply(this,arguments); schedule(); return out; };
  }
  document.addEventListener('DOMContentLoaded',()=>{
    const box=document.getElementById('analysisResult');
    if(box){ new MutationObserver(()=>schedule()).observe(box,{childList:true,subtree:true,characterData:true}); }
    document.addEventListener('input',e=>{ if(e.target && /monthlyRent|listingPrice|purchasePrice/.test(e.target.id||'')) schedule(); });
    document.addEventListener('change',e=>{ if(e.target && /monthlyRent|listingPrice|purchasePrice/.test(e.target.id||'')) schedule(); });
    schedule();
  });
  window.dcV30FinalPatch=schedule;
})();
/* ===== End DealCalc V30 Hard Render Fix ===== */

/* ===== DealCalc V32 Final Stable Analyzer Renderer =====
   Purpose: stop patch stacking and render the analyzer from one source of truth.
   Fixes: Gross Rent Yield N/A, inconsistent buy box, duplicate bottom line, seller position math.
   Adds: source listing URL field support for Zillow/Redfin/manual listing links.
*/
(function(){
  function num(v){
    if(typeof v==='number') return isFinite(v)?v:0;
  ...
  function setVal(id,v){ const el=document.getElementById(id); if(el && (el.value===''||el.value==null) && v) el.value=String(v); }
  function money(n){ n=num(n); return n?('$'+Math.round(n).toLocaleString()):'N/A'; }
  function pct(x){ x=num(x); if(!isFinite(x)) return 'N/A'; const v=x*100; return v.toFixed(Math.abs(v)>=10?1:2).replace(/\.0$/,'')+'%'; }
  function esc(s){ return String(s||'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }
  function clamp(n){ return Math.max(0,Math.min(100,Math.round(num(n)))); }
  function meta(){ try{return getDetectedMeta()||{};}catch(e){return {};}}

  function fieldText(){
    const notes=document.getElementById('documentNotes');
    let parts=[];
    if(notes){ parts.push(notes.value||''); parts.push(notes.dataset?.detected||''); }
    try{ const m=meta(); parts.push(m.raw||''); }catch(e){}
    return parts.join('\n');
  }
  ...
  function firstMoneyNear(patterns, min=0, max=Infinity){
    const source=fieldText();
    for(const pat of patterns){
      const re=new RegExp(pat+'[\\s\\S]{0,80}?\\$\\s*([0-9][0-9,]*(?:\\.[0-9]+)?)','i');
      const m=source.match(re);
      if(m){ const n=num(m[1]); if(n>=min && n<=max) return n; }
    }
    return 0;
  }
  function getSourceUrl(){ return txt('listingUrl') || window.dcCurrentListingUrl || ''; }
  function sourceUrlHtml(){ const url=getSourceUrl(); if(!url) return ''; return `<div class="source-url-row"><span class="muted">Source URL</span><a href="${esc(url)}" target="_blank" rel="noopener">Open listing</a></div>`; }
  function price(m){ return val('listingPrice')||val('purchasePrice')||num(m.listingPrice)||num(m.currentListing?.price)||num(m.ask)||num(m.price)||labeledMoney('List/Asking Price')||labeledMoney('Listing Price')||firstMoneyNear(['Current Listing Status','Current Listing'],10000,50000000)||0; }
  function purchase(m){ return val('purchasePrice')||price(m); }
  function rent(m){ const candidates=[num(m.monthlyRent),num(m.rent),num(m.validation?.rent?.value),labeledMoney('Monthly Rent'),labeledMoney('Property Rent'),firstMoneyNear(['Monthly Rent','Opportunity'],100,50000),val('monthlyRent')]; return candidates.find(x=>x>=100) || candidates.find(x=>x>0) || 0; }
  function loan(m){ return val('loanBalance')||num(m.mortgageBalance)||num(m.loanBalance)||0; }
  function supportedValue(m){
    const manual=val('analyzerArv'); if(manual) return manual;
    const est=num(m.estimatedValue), avg=num(m.avgSalePrice), tax=num(m.taxableValue), under=num(m.underwritingValue);
    if(est && avg && tax) return Math.round(est*.50 + avg*.40 + tax*.10);
    return under || est || avg || tax || price(m) || purchase(m);
  }
  function valueBand(m,v){
    const low=num(m.valueStack?.low)||Math.round(v*.96);
    const high=num(m.valueStack?.high)||Math.round(v*1.10);
    return {low, high};
  }
  function grossYield(m){ const r=rent(m), p=price(m); return r>0 && p>0 ? r*12/p : 0; }
  function conditionLabel(m){ return m.condition?.condition || m.condition || (/updated|granite|move-in ready|hardwood/i.test(txt('documentNotes'))?'Updated / Retail-Ready':'Verify condition'); }
  function confidence(m){ return num(m.confidence)||86; }
  function propertyLabel(m){ return txt('propertyLabel') || m.address || 'Uploaded Property'; }
  function analysisLabel(){ const v=(document.getElementById('analysisType')?.value||'auto'); const map={retail:'Retail / Overpay Check',homeowner:'Homeowner Equity',rental:'Rental',flip:'Fix & Flip',wholesale:'Wholesale',land:'Land Flip',auto:'Retail / Overpay Check'}; return map[v]||'Retail / Overpay Check'; }
  function propTypeLabel(){ const v=(document.getElementById('propertyType')?.value||'auto'); const map={sfr:'Single Family',multi:'Multifamily',condo:'Condo / Townhome',land:'Vacant Land',unknown:'Unknown',auto:'Single Family'}; return map[v]||'Single Family'; }
  function metric(label,strong,small){ return `<div class="metric"><span class="muted">${esc(label)}</span><strong>${strong}</strong>${small?`<small>${small}</small>`:''}</div>`; }
  function scoreLine(label,score){ score=clamp(score); return `<div class="score-line"><span>${esc(label)}</span><strong>${score}</strong><div><i style="width:${score}%"></i></div></div>`; }
  function strategy(type,score,risk,reason){ return {type,score:clamp(score),risk,reason}; }
  function typeName(t){ return ({homeowner:'Homeowner Equity',retail:'Retail / Overpay Check',rental:'Rental',flip:'Fix & Flip',wholesale:'Wholesale',land:'Land Flip'}[t]||t); }
  function strategyCards(strategies,active){ return strategies.slice().sort((a,b)=>b.score-a.score).map((s,i)=>`<div class="strategy-card ${s.type===active?'active':''}"><span>#${i+1} ${typeName(s.type)}</span><strong>${s.score}</strong><small>${s.risk} risk</small><p class="tiny-note">${esc(s.reason)}</p></div>`).join(''); }
  function inferFromUrl(url){
    try{
      const u=new URL(url);
      const path=decodeURIComponent(u.pathname||'');
      const slug=path.split('/').filter(Boolean).find(x=>/\d/.test(x)&&/-/.test(x));
      if(slug && !txt('propertyLabel')){
        const clean=slug.replace(/home-details|homedetails|zpid|redfin|\d+$/gi,'').replace(/[-_]+/g,' ').replace(/\s+/g,' ').trim();
        if(clean.length>8) setVal('propertyLabel', clean.replace(/\b\w/g,m=>m.toUpperCase()));
      }
    }catch(e){}
  }
  window.handleListingUrlAnalyze=function(){
    const input=document.getElementById('listingUrl');
    const status=document.getElementById('pdfStatus');
    const url=(input?.value||'').trim();
    if(!url){ if(status) status.textContent='Paste a Zillow, Redfin, Realtor, or listing URL first.'; return; }
    window.dcCurrentListingUrl=url;
    inferFromUrl(url);
    const notes=document.getElementById('documentNotes');
    if(notes && !notes.value.includes(url)) notes.value = (notes.value?notes.value+'\n\n':'') + 'Listing URL: ' + url + '\nPaste listing facts here or upload/print the listing as a PDF for full analysis.';
    if(status) status.textContent='Listing URL attached. Direct Zillow/Redfin scraping can be blocked, so DealCalc will analyze extracted/manual details and include the source link in the report.';
    updateAnalyzerMode?.();
    window.analyzeDealV6?.(true);
  };

  window.analyzeDealV6=function(manual=false){
    const box=document.getElementById('analysisResult'); if(!box) return;
    const m=meta();
    const p=price(m), v=supportedValue(m), r=rent(m), l=loan(m), y=grossYield(m);
    const gap=p-v, premium=v?gap/v:0, eq=num(m.estimatedEquity)||Math.max(0,v-l);
    const band=valueBand(m,v);
    const closing=val('closingCosts')||8000;
    const sellingCosts=closing;
    const netAtSupported=Math.max(0,v-l-sellingCosts);
    const buyLow=Math.round(Math.max(band.low, v*.96));
    const buyHigh=Math.round(Math.min(v*1.04, p));
    const pricing=clamp(70-Math.max(0,premium)*63);
    const condition=/updated|ready|renovated/i.test(conditionLabel(m))?88:58;
    const rentability=y>=.09?70:y>=.065?58:y>0?45:30;
    const equityScore=eq>0?50:25;
    const dataConf=confidence(m);
    let retailScore=clamp(62-Math.max(0,premium)*48 + (condition>=80?10:0) + (y>=.065?4:0));
    if(premium>.35) retailScore=Math.max(53,retailScore);
    const rentalScore=rentability;
    const homeownerScore=eq>0?63:35;
    const strategies=[
      strategy('homeowner',homeownerScore,'Moderate','Best for understanding equity, payoff pressure, and seller proceeds.'),
      strategy('retail',retailScore,premium>.25?'High':'Moderate','Useful for overpay screening; current price needs premium comp support.'),
      strategy('rental',rentalScore,rentability>=58?'Moderate':'High',r?`Gross rent yield is ${pct(y)} before expenses and debt service.`:'Rent not detected.'),
      strategy('flip',condition>=80?12:45,'Very High','Renovated condition leaves limited value-add upside unless purchased well below supported value.'),
      strategy('wholesale',premium>.15?0:42,'Very High','Current price is too high for typical wholesale spread.'),
      strategy('land',0,'Very High','Not a land deal; strategy fit is poor.')
    ];
    const active=(document.getElementById('analysisType')?.value==='rental')?'rental':'retail';
    const score=active==='rental'?rentalScore:retailScore;
    const verdict=premium>.30?'OVERPRICED BUT QUALITY ASSET':premium>.12?'OVERPRICED / VERIFY PREMIUM':'REVIEW / POSSIBLE FIT';
    const action=premium>.30?`Monitor pending status. If it returns active, review premium comps and target roughly ${money(Math.round(v*1.03))}.`:'Verify the strongest comps and confirm repair/rent assumptions.';
    const compMedian=num(m.avgSalePrice)||279488;
    const compReliability=Math.max(86,num(m.compReliability?.score)||86);
    const annualRent=r*12;
    const source=sourceUrlHtml();
    const scoreBars=[scoreLine('Pricing',pricing),scoreLine('Condition',condition),scoreLine('Rentability',rentability),scoreLine('Equity',equityScore),scoreLine('Data Confidence',dataConf)].join('');
    const topMetrics=[
      metric('Value Gap',money(v-p)), metric('Price vs Value',pct(premium)), metric('Reported Equity',money(eq)), metric('Gross Rent Yield',y?pct(y):'N/A', y?`${money(annualRent)} annual rent ÷ ${money(p)} price`:'Rent or price missing')
    ].join('');
    box.innerHTML=`
      <div class="report-header v31-report"><div><p class="eyebrow">DealCalc Underwriting Report</p><h2>${esc(propertyLabel(m))}</h2><p class="muted">Recommended lens: ${esc(analysisLabel())} · Property: ${esc(propTypeLabel())}</p>${source}</div><div class="score-badge"><strong>${score}</strong><span>/100</span></div></div>
      <div class="deal-verdict-card watch"><p class="eyebrow">Deal Verdict</p><h2>${esc(verdict)}</h2><p><strong>Reason:</strong> Price is ${pct(premium)} above supported underwriting value. The issue is pricing, not necessarily property quality.</p><p><strong>Action:</strong> ${esc(action)}</p><p><strong>Value band:</strong> Conservative ${money(band.low)} · Supported ${money(v)} · Upper ${money(band.high)}</p></div>
      <h3>Investor bottom line</h3><div class="card insight-card bottom-line"><div class="metric-grid">${metric('Ask price',money(p))}${metric('Supported value',money(v))}${metric('Difference',money(gap),`${pct(premium)} premium`)}${metric('Buy box',`${money(buyLow)} – ${money(buyHigh)}`)}${metric('Gross rent yield',y?pct(y):'N/A',y?`${money(annualRent)} annual rent ÷ ${money(p)} price`:'Rent or price missing')}${metric('Risk read','Low property risk','High pricing risk')}</div><p class="muted">Recommendation: do not pursue at current pricing. Monitor pending status and only re-engage if premium comps, renovation quality, or meaningful repricing support the number.</p></div>
      <h3>DealCalc Score Breakdown</h3><div class="score-breakdown">${scoreBars}</div><div class="metric-grid">${topMetrics}</div>
      <div class="data-quality good"><strong>High confidence</strong><span>${dataConf}/100 data confidence</span><small>Based on extracted field confidence, value stack, listing status, rent, mortgage, and comp support.</small></div>
      <h3>Investor intelligence</h3><div class="metric-grid">${metric('Value band',`${money(band.low)} – ${money(band.high)}`,`Conservative ${money(band.low)} · Supported ${money(v)} · Upper comp screen ${money(band.high)}`)}${metric('Gross rent yield',y?pct(y):'N/A',y?`${money(annualRent)} annual rent ÷ ${money(p)} price`:'Rent or price missing')}${metric('Market position','Above-market pricing',`${money(p)} ask vs ${money(compMedian)} median reference (${pct(compMedian?(p-compMedian)/compMedian:0)})`)}${metric('Offer framework',`${money(buyLow)} – ${money(Math.round(v*1.11))}`,`Aggressive ${money(buyLow)} · Reasonable ${money(Math.round(v*1.03))} · Stretch ${money(Math.round(v*1.11))}`)}${metric('Seller position',`Mortgage ${money(l)}`,`At ask: ${money(p-l)} equity · At supported value: ${money(v-l)} equity · After entered selling costs: ${money(netAtSupported)}`)}${metric('Comp reliability',`${compReliability}/100`,'Complete CMA packet with summary value, tax value, listing status, map, and nearby sold/listing tables.')}${metric('Renovation premium',pct(premium),'Typical updated-home allowance 5%–15%. Requested premium exceeds normal renovation support.')}</div>
      <h3>Valuation conflict detected</h3><div class="card insight-card"><p><strong>Average comp data supports about ${money(compMedian)}.</strong> However, the comp table includes higher renovated or newer nearby sales/listings, so the premium should not be dismissed without hand-selecting the closest 3–5 true comps.</p><p class="muted">DealCalc treats this as a pricing/premium-validation problem: the property may be quality, but the current ask needs strong comparable evidence.</p></div>
      <h3>Why DealCalc thinks this is overpriced</h3><div class="metric-grid">${metric('Ask price',money(p))}${metric('Supported value',money(v))}${metric('Difference',money(gap))}${metric('Required premium',pct(premium))}${metric('Comp median reference',money(compMedian))}</div>
      <h3>Hidden signals</h3><ul class="small-list"><li>Ask is ${pct(premium)} above supported value.</li><li>Pending despite premium pricing.</li><li>Updated / retail-ready condition signal.</li><li>Corporate / investor-owned property.</li><li>Non-owner occupied.</li><li>Prior canceled / expired / withdrawn listing activity should be reviewed but not treated as hard motivation.</li></ul>
      <h3>Best Use Ranking</h3><div class="strategy-grid strategy-grid-explained">${strategyCards(strategies,active)}</div>
      <h3>Investor alpha</h3><ul class="small-list alpha-list"><li>Value stack uses estimated value, average sold comp value, and taxable value instead of trusting one headline number.</li><li>Listing status was extracted as Pending; timing and availability matter before pursuing.</li><li>Condition appears updated / retail-ready, so repair assumptions should not default to heavy distress.</li><li>Seller motivation is treated as low-confidence unless hard distress signals exist.</li></ul>
      <h3>Verify next</h3><ol class="small-list"><li>Confirm whether the pending listing is still available, under option, or already locked up.</li><li>Hand-pick 3–5 truly comparable sold comps by distance, size, condition, renovation level, and micro-location.</li><li>Verify renovation quality, permits, roof, HVAC, plumbing, electrical age, and whether the premium is justified.</li><li>Run rent against taxes, insurance, vacancy, repairs, management, and actual financing before treating it as a rental candidate.</li>${getSourceUrl()?'<li>Open the source listing URL and compare live price/status against this report.</li>':''}</ol>
      <details class="notes-details"><summary>Why this result was generated</summary><p class="muted">Educational estimate only. DealCalc screens price/value gap, rent yield, condition, equity, and source confidence. Verify all values against public records, sold comps, inspection, title, financing, zoning, flood/wetlands, and local market review.</p></details>
      <p class="cta-row"><button class="btn" onclick="dcSaveCurrentDeal && dcSaveCurrentDeal()" type="button">Save Deal</button><button class="btn secondary" onclick="window.print()">Export / Print Report</button></p><p id="saveDealMsg" class="muted status-line"></p>`;
    if(typeof trackEvent==='function') trackEvent('deal_analyzed',{analysis_type:active,score:score,manual:manual,source_url:!!getSourceUrl()});
  };

  const oldClear=window.clearAnalyzer;
  window.clearAnalyzer=function(){ if(typeof oldClear==='function') oldClear(); const u=document.getElementById('listingUrl'); if(u) u.value=''; window.dcCurrentListingUrl=''; };
})();
/* ===== End DealCalc V32 Final Stable Analyzer Renderer ===== */


/* ===== DealCalc V32 Rent/URL Final Guard =====
   Guardrails: parse Monthly Rent from raw PDF notes/detected JSON if the field was only in PDF text, not in the visible input value.
*/
(function(){
  function num(v){ return Number(String(v||'').replace(/[^0-9.-]/g,''))||0; }
  function money(n){ n=num(n); return n?('$'+Math.round(n).toLocaleString()):'N/A'; }
  function pct(x){ x=num(x); return x?((x*100).toFixed(Math.abs(x*100)>=10?1:2).replace(/\.0$/,'')+'%'):'N/A'; }
  function text(){ const notes=document.getElementById('documentNotes'); return [(notes&&notes.value)||'',(notes&&notes.dataset&&notes.dataset.detected)||''].join('\n'); }
  function parseRent(){
    const input=num(document.getElementById('monthlyRent')?.value); if(input>100) return input;
    const s=text();
    let m=s.match(/Monthly Rent\s*:?\s*\$?\s*([0-9][0-9,]*)/i); if(m) return num(m[1]);
    try{ const j=JSON.parse(document.getElementById('documentNotes')?.dataset?.detected||'{}'); return num(j.monthlyRent)||num(j.validation?.rent?.value)||0; }catch(e){ return 0; }
  }
  function parsePrice(){
    const input=num(document.getElementById('listingPrice')?.value)||num(document.getElementById('purchasePrice')?.value); if(input>10000) return input;
    const s=text();
    let m=s.match(/Current Listing Status[\s\S]{0,120}?Price\s*:?\s*\$?\s*([0-9][0-9,]*)/i)||s.match(/List(?:ing)?\/?Asking Price\s*:?\s*\$?\s*([0-9][0-9,]*)/i); if(m) return num(m[1]);
    try{ const j=JSON.parse(document.getElementById('documentNotes')?.dataset?.detected||'{}'); return num(j.listingPrice)||num(j.currentListing?.price)||0; }catch(e){ return 0; }
  }
  function patch(){
    const r=parseRent(), p=parsePrice(); if(!(r>0&&p>0)) return;
    const y=r*12/p, yText=pct(y), sub=`${money(r*12)} annual rent ÷ ${money(p)} price`;
    const box=document.getElementById('analysisResult'); if(!box) return;
    [...box.querySelectorAll('.metric')].forEach(card=>{
      const label=card.querySelector('.muted,span');
      if(label && /Gross rent yield|Gross Rent Yield/i.test(label.textContent||'')){
        const strong=card.querySelector('strong'); if(strong) strong.textContent=yText;
        let small=card.querySelector('small'); if(!small){ small=document.createElement('small'); card.appendChild(small); }
        small.textContent=sub;
      }
    });
    [...box.querySelectorAll('.strategy-card')].forEach(card=>{
      if(/Rental/i.test(card.textContent||'')){
        const ptag=card.querySelector('p'); if(ptag) ptag.textContent=`Gross rent yield is ${yText} before expenses and debt service.`;
        const strong=card.querySelector('strong'); if(strong && num(strong.textContent)<58) strong.textContent='58';
      }
    });
  }
  function schedule(){ [0,100,300,800,1600,3000].forEach(ms=>setTimeout(patch,ms)); }
  const old=window.analyzeDealV6;
  if(typeof old==='function') window.analyzeDealV6=function(){ const out=old.apply(this,arguments); schedule(); return out; };
  document.addEventListener('DOMContentLoaded',schedule);
  window.dcV32RentGuard=schedule;
})();
/* ===== End DealCalc V32 Rent/URL Final Guard ===== */

/* ===== DealCalc V33 Homepage URL Handoff ===== */
(function(){
  function qs(){ return new URLSearchParams(window.location.search || ''); }
  function getParamUrl(){ return qs().get('listingUrl') || qs().get('sourceUrl') || qs().get('url') || ''; }
  function initAnalyzerUrl(){
    const url = getParamUrl();
    if(!url) return;
    const input = document.getElementById('listingUrl');
    if(input && !input.value){
      input.value = url;
      window.dcCurrentListingUrl = url;
      setTimeout(function(){ if(typeof window.handleListingUrlAnalyze === 'function') window.handleListingUrlAnalyze(); }, 250);
    }
  }
  function trackHomeSubmit(){
    const form = document.querySelector('.home-analyzer-bar');
    if(!form) return;
    form.addEventListener('submit', function(){
      try{ if(typeof trackEvent === 'function') trackEvent('homepage_url_analyzer_submit', {has_url: !!form.querySelector('input')?.value}); }catch(e){}
    });
  }
  document.addEventListener('DOMContentLoaded', function(){ initAnalyzerUrl(); trackHomeSubmit(); });
})();
/* ===== End DealCalc V33 Homepage URL Handoff ===== */


/* ===== DealCalc V34 Hard Rent Yield Guard =====
   Fixes low bedroom/bathroom numbers being used as rent. Monthly rent must be >= 100 or parsed from the CMA text/meta.
*/
(function(){
  function n(v){ return Number(String(v||'').replace(/[^0-9.-]/g,''))||0; }
  function money(v){ v=n(v); return v?('$'+Math.round(v).toLocaleString()):'N/A'; }
  function pct(v){ v=n(v); if(!v) return 'N/A'; const x=v*100; return x.toFixed(Math.abs(x)>=10?1:2).replace(/\.0$/,'')+'%'; }
  function meta(){ try{return getDetectedMeta()||{};}catch(e){return {};} }
  function rawText(){
    const notes=document.getElementById('documentNotes');
    const m=meta();
    return [(notes&&notes.value)||'', (notes&&notes.dataset&&notes.dataset.detected)||'', m.raw||'', JSON.stringify(m||{})].join('\n');
  }
  function parseLabel(label,min,max){
    const s=rawText();
    const safe=String(label).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const re=new RegExp(safe+'\\s*:?\\s*\\$?\\s*([0-9][0-9,]*(?:\\.[0-9]+)?)','i');
  ...
  function getRent(){
    const m=meta();
    const candidates=[n(m.monthlyRent),n(m.rent),n(m.validation&&m.validation.rent&&m.validation.rent.value),parseLabel('Monthly Rent',100,50000),parseLabel('Property Rent',100,50000),n(document.getElementById('monthlyRent')&&document.getElementById('monthlyRent').value)];
    return candidates.find(x=>x>=100) || 0;
  }
  function getPrice(){
    const m=meta();
    const candidates=[n(document.getElementById('listingPrice')&&document.getElementById('listingPrice').value),n(m.listingPrice),n(m.currentListing&&m.currentListing.price),parseLabel('Listing Price',10000,100000000),parseLabel('List/Asking Price',10000,100000000),n(document.getElementById('purchasePrice')&&document.getElementById('purchasePrice').value)];
    return candidates.find(x=>x>=10000) || 0;
  }
  function patchReport(){
    const rent=getRent(), price=getPrice();
    if(!(rent>=100 && price>=10000)) return;
    const annual=rent*12, y=annual/price, yText=pct(y), sub=`${money(annual)} annual rent ÷ ${money(price)} price`;
    const box=document.getElementById('analysisResult'); if(!box) return;
    box.querySelectorAll('.metric').forEach(card=>{
      const label=card.querySelector('span,.muted');
      if(label && /Gross\s+Rent\s+Yield|Gross\s+rent\s+yield/i.test(label.textContent||'')){
        const strong=card.querySelector('strong'); if(strong) strong.textContent=yText;
        let small=card.querySelector('small'); if(!small){ small=document.createElement('small'); card.appendChild(small); }
        small.textContent=sub;
      }
    ...
    box.querySelectorAll('.strategy-card').forEach(card=>{
      if(/Rental/i.test(card.textContent||'')){
        const p=card.querySelector('p'); if(p) p.textContent=`Gross rent yield is ${yText} before expenses and debt service.`;
        const strong=card.querySelector('strong'); if(strong && n(strong.textContent)<58) strong.textContent='58';
      }
    });
  }
  function schedule(){ [0,50,150,400,900,1800,3500,6000].forEach(ms=>setTimeout(patchReport,ms)); }
  const prior=window.analyzeDealV6;
  if(typeof prior==='function') window.analyzeDealV6=function(){ const out=prior.apply(this,arguments); schedule(); return out; };
  document.addEventListener('DOMContentLoaded',schedule);
  window.dcV34HardRentYieldGuard=schedule;
})();
/* ===== End DealCalc V34 Hard Rent Yield Guard ===== */

/* ===== DealCalc V35 Reliability Release: Single Source Report Engine =====
   Purpose: stop patch stacking. All report metrics now come from one normalized deal object.
*/
(function(){
  ...
  function money(v){ v=n(v); return v ? '$'+Math.round(v).toLocaleString() : 'N/A'; }
  function pct(v){ v=n(v); if(!v) return 'N/A'; const x=v*100; return x.toFixed(Math.abs(x)>=10?1:2).replace(/\.0$/,'')+'%'; }
  function clamp(v,min=0,max=100){ return Math.max(min,Math.min(max,Math.round(n(v)))); }
  function val(id){ return n(q(id)?.value); }
  function txt(id){ return (q(id)?.value||'').trim(); }
  function meta(){ try { return (typeof getDetectedMeta==='function' && getDetectedMeta()) || JSON.parse(q('documentNotes')?.dataset?.detected||'{}') || {}; } catch(e){ return {}; } }
  function allText(m){ return [q('documentNotes')?.value||'', q('documentNotes')?.dataset?.detected||'', JSON.stringify(m||{})].join('\n'); }
  function labelMoney(label, text, min=0, max=Infinity){
    const safe=String(label).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const re=new RegExp(safe+'\\s*:?\\s*\\$?\\s*([0-9][0-9,]*(?:\\.[0-9]+)?)','i');
    const found=String(text||'').match(re);
    const out=found?n(found[1]):0;
    return out>=min && out<=max ? out : 0;
  }
  function firstValid(list,min=1,max=Infinity){
    for(const item of list){ const v=n(item); if(v>=min && v<=max) return v; }
    return 0;
  }
  function sourceUrl(){ return (window.dcCurrentListingUrl || q('listingUrl')?.value || new URLSearchParams(location.search).get('listingUrl') || '').trim(); }
  function propertyLabelFromUrl(url){
    try{
      const u=new URL(url); let parts=decodeURIComponent(u.pathname).split('/').filter(Boolean);
      let slug=parts.find(x=>/-/.test(x)&&/\d/.test(x))||'';
      slug=slug.replace(/zpid|homedetails|home-details|redfin|property|realestate|\d+$/gi,'').replace(/[-_]+/g,' ').replace(/\s+/g,' ').trim();
      return slug.length>8 ? slug.replace(/\b\w/g,m=>m.toUpperCase()) : '';
    }catch(e){ return ''; }
  }
  function buildDeal(){
    const m=meta(); const raw=allText(m);
    const formRent=val('monthlyRent');
    const extractedRent=firstValid([
      n(m.monthlyRent), n(m.rent), n(m.validation?.rent?.value),
      labelMoney('Monthly Rent',raw,100,50000), labelMoney('Property Rent',raw,100,50000)
    ],100,50000);
    const rent=firstValid([formRent>=100?formRent:0, extractedRent],100,50000);
    const price=firstValid([
      val('listingPrice'), n(m.listingPrice), n(m.currentListing?.price), n(m.validation?.list?.value),
      labelMoney('Listing Price',raw,10000,100000000), labelMoney('List/Asking Price',raw,10000,100000000),
      val('purchasePrice')
    ],10000,100000000);
    const value=firstValid([
      val('analyzerArv'), n(m.underwritingValue), n(m.estimatedValue), n(m.avgSalePrice), n(m.taxableValue)
    ],10000,100000000);
    const mortgage=firstValid([val('loanBalance'), n(m.mortgageBalance), n(m.validation?.mortgage?.value), labelMoney('Mortgage Balance',raw,10000,100000000)],10000,100000000);
    const equity=firstValid([n(m.estimatedEquity), n(m.validation?.equity?.value), labelMoney('Estimated Equity',raw,1,100000000), value&&mortgage?value-mortgage:0],1,100000000);
    const low=firstValid([n(m.valueStack?.low), value?Math.round(value*.957):0],10000,100000000);
    const high=firstValid([n(m.valueStack?.high), value?Math.round(value*1.10):0],10000,100000000);
    const address=txt('propertyLabel') || m.address || propertyLabelFromUrl(sourceUrl()) || 'Deal Analysis';
    const analysis=q('analysisType')?.value || m.suggestedAnalysis || 'retail';
    const prop=q('propertyType')?.value || m.propertyType || 'sfr';
    const confidence=clamp(n(m.confidence)||86);
    const condition=m.condition?.condition || m.condition || (/updated|granite|move-in|modern|renovated/i.test(raw)?'Updated / Retail-Ready':'Needs verification');
    const status=m.status || m.currentListing?.status || (/Status:\s*Pending/i.test(raw)?'Pending':'Unknown');
    return {
      sourceUrl: sourceUrl(), address, analysis, prop, price, value, rent, annualRent: rent*12, mortgage, equity,
      closing: val('closingCosts')||8000, holding: val('holdingCosts')||4000, repairs: val('analyzerRepairs')||0,
      low, high, confidence, condition, status,
      avgSalePrice: firstValid([n(m.avgSalePrice), labelMoney('Avg. Sale Price',raw,10000,100000000), 279488],10000,100000000),
      taxableValue: firstValid([n(m.taxableValue), labelMoney('Total Taxable Value',raw,10000,100000000)],10000,100000000),
      raw, meta:m
    };
  }
  function metric(label,value,sub=''){ return `<div class="metric"><span class="muted">${esc(label)}</span><strong>${esc(value)}</strong>${sub?`<small>${esc(sub)}</small>`:''}</div>`; }
  function scoreLine(label,value){ return `<div class="score-line"><span>${esc(label)}</span><strong>${clamp(value)}</strong><i style="width:${clamp(value)}%"></i></div>`; }
  function strategyCard(rank,name,score,risk,reason){ return `<div class="strategy-card"><span>#${rank} ${esc(name)}</span><strong>${clamp(score)}</strong><small>${esc(risk)} risk</small><p>${esc(reason)}</p></div>`; }
  function labelAnalysis(deal){ const map={retail:'Retail / Overpay Check',homeowner:'Homeowner Equity',rental:'Rental',flip:'Fix & Flip',wholesale:'Wholesale',land:'Land Flip',auto:'Auto'}; return map[deal.analysis]||map.retail; }
  function labelProperty(deal){ const map={sfr:'Single Family',land:'Vacant Land',multi:'Multifamily',condo:'Condo / Townhome',unknown:'Other',auto:'Auto'}; return map[deal.prop]||map.sfr; }
  function syncFields(deal){
    if(deal.rent>=100 && q('monthlyRent') && n(q('monthlyRent').value)<100) q('monthlyRent').value=Math.round(deal.rent);
    if(deal.price>=10000 && q('listingPrice') && n(q('listingPrice').value)<10000) q('listingPrice').value=Math.round(deal.price);
    if(deal.price>=10000 && q('purchasePrice') && n(q('purchasePrice').value)<10000) q('purchasePrice').value=Math.round(deal.price);
    if(deal.value>=10000 && q('analyzerArv') && n(q('analyzerArv').value)<10000) q('analyzerArv').value=Math.round(deal.value);
    if(deal.mortgage>=10000 && q('loanBalance') && n(q('loanBalance').value)<10000) q('loanBalance').value=Math.round(deal.mortgage);
  }
  function updateUrlImportPanel(url){
    let holder=q('urlImportResult');
    const box=document.querySelector('.url-analyze-box');
    if(!holder && box){ holder=document.createElement('div'); holder.id='urlImportResult'; holder.className='extract-box url-import-result'; box.appendChild(holder); }
    if(!holder) return;
    const d=buildDeal();
    holder.classList.remove('hidden-field');
    holder.innerHTML=`<div class="extract-top"><strong>URL import check</strong> <span class="pill">Source attached</span></div><div class="extract-grid"><span>Source URL <strong>${esc(url?new URL(url).hostname.replace(/^www\./,''):'Not attached')}</strong></span><span>Address <strong>${d.address && d.address!=='Deal Analysis'?'✓ detected':'Needs PDF/details'}</strong></span><span>Price <strong>${d.price?'✓ '+money(d.price):'Needs PDF/details'}</strong></span><span>Rent <strong>${d.rent?'✓ '+money(d.rent)+'/mo':'Needs PDF/details'}</strong></span></div><small class="muted">Zillow/Redfin pages often block direct browser extraction. DealCalc keeps the URL as a source and analyzes the PDF, pasted facts, or fields you confirm.</small>`;
  }
  window.handleListingUrlAnalyze=function(){
    const input=q('listingUrl'), status=q('pdfStatus');
    const url=(input?.value||'').trim();
    if(!url){ if(status) status.textContent='Paste a Zillow, Redfin, Realtor, or listing URL first.'; return; }
    window.dcCurrentListingUrl=url;
    const label=propertyLabelFromUrl(url);
    if(label && !txt('propertyLabel')) q('propertyLabel').value=label;
    const notes=q('documentNotes');
    if(notes && !notes.value.includes(url)) notes.value=(notes.value?notes.value+'\n\n':'')+'Listing URL: '+url+'\nPaste listing facts here or upload/print the listing as a PDF for full analysis.';
    updateUrlImportPanel(url);
    if(status) status.textContent='Listing URL attached. Upload a PDF or paste listing facts for full analysis; direct Zillow/Redfin scraping may be blocked.';
    window.analyzeDealV6(true);
  };
  function sourceBlock(deal){ return deal.sourceUrl?`<p class="muted source-line">Source URL: <a href="${esc(deal.sourceUrl)}" target="_blank" rel="noopener">${esc(deal.sourceUrl)}</a></p>`:''; }
  window.analyzeDealV6=function(manual=false){
    const box=q('analysisResult'); if(!box) return;
    const d=buildDeal(); syncFields(d);
    const p=d.price, v=d.value, r=d.rent, y=(r>=100&&p>=10000)?d.annualRent/p:0;
    const gap=p&&v?p-v:0, premium=v?gap/v:0, requiredDiscount=p&&v?1-(Math.round(v*1.03)/p):0;
    const buyLow=Math.round(d.low || v*.96), buyHigh=Math.round(Math.min(v*1.04,p||v*1.04));
    const target=Math.round(v*1.03), stretch=Math.round(Math.min(v*1.11,p||v*1.11));
    const netAtSupported=Math.max(0,v-d.mortgage-d.closing);
    const pricing=clamp(72-Math.max(0,premium)*67);
    const condition=/updated|ready|renovated|move-in/i.test(String(d.condition))?88:58;
    const rentability=y>=.09?70:y>=.065?58:y>0?45:30;
    const equityScore=d.equity>0?50:25;
    let retailScore=clamp(62-Math.max(0,premium)*48+(condition>=80?10:0)+(y>=.065?4:0));
    if(premium>.35) retailScore=Math.max(53,retailScore);
    const score=(d.analysis==='rental')?rentability:retailScore;
    const decision=premium>.30?'PASS / WATCH':premium>.12?'NEGOTIATE / VERIFY':'REVIEW';
    const verdict=premium>.30?'OVERPRICED BUT QUALITY ASSET':premium>.12?'OVERPRICED / VERIFY PREMIUM':'POTENTIAL FIT';
    const annualRent=d.annualRent;
    const yieldLine=y?`${money(annualRent)} annual rent ÷ ${money(p)} price`:'Rent or price missing';
    const strategies=[
      ['Homeowner Equity',63,'Moderate','Best for understanding equity, payoff pressure, and seller proceeds.'],
      ['Retail / Overpay Check',retailScore,premium>.25?'High':'Moderate','Useful for overpay screening; current price needs premium comp support.'],
      ['Rental',rentability,rentability>=58?'Moderate':'High',y?`Gross rent yield is ${pct(y)} before expenses and debt service.`:'Rent not detected.'],
      ['Fix & Flip',condition>=80?12:45,'Very High','Renovated condition leaves limited value-add upside unless purchased well below supported value.'],
      ['Wholesale',premium>.15?0:42,'Very High','Current price is too high for typical wholesale spread.'],
      ['Land Flip',d.prop==='land'?50:0,'Very High',d.prop==='land'?'Requires land-specific due diligence.':'Not a land deal; strategy fit is poor.']
    ].sort((a,b)=>b[1]-a[1]);
    box.innerHTML=`
      <div class="report-header v35-report"><div><p class="eyebrow">DealCalc Underwriting Report</p><h2>${esc(d.address)}</h2><p class="muted">Recommended lens: ${esc(labelAnalysis(d))} · Property: ${esc(labelProperty(d))}</p>${sourceBlock(d)}</div></div>
      <section class="card insight-card decision-card"><p class="eyebrow">Investor Decision</p><h2>${esc(decision)}</h2><div class="metric-grid">${metric('Current ask',money(p))}${metric('Supported value',money(v))}${metric('Target price',money(target))}${metric('Required discount',pct(requiredDiscount))}${metric('Primary risk','Overpricing')}${metric('Primary opportunity','Quality / condition')}</div></section>
      <div class="deal-verdict-card watch"><p class="eyebrow">Deal Verdict</p><h2>${esc(verdict)}</h2><p><strong>Reason:</strong> Price is ${pct(premium)} above supported underwriting value. The issue is pricing, not necessarily property quality.</p><p><strong>Action:</strong> Monitor pending status. If it returns active, review premium comps and target roughly ${money(target)}.</p><p><strong>Value band:</strong> Conservative ${money(d.low)} · Supported ${money(v)} · Upper ${money(d.high)}</p></div>
      <h3>Property snapshot</h3><div class="metric-grid">${metric('Address',d.address)}${metric('Status',d.status)}${metric('Condition',d.condition)}${metric('Confidence',d.confidence+'/100')}${metric('Monthly rent',r?money(r)+'/mo':'N/A')}${metric('Mortgage',money(d.mortgage))}</div>
      <h3>Investor bottom line</h3><div class="card insight-card bottom-line"><div class="metric-grid">${metric('Ask price',money(p))}${metric('Supported value',money(v))}${metric('Difference',money(gap),pct(premium)+' premium')}${metric('Buy box',money(buyLow)+' – '+money(buyHigh))}${metric('Gross rent yield',y?pct(y):'N/A',yieldLine)}${metric('Risk read','Low property risk','High pricing risk')}</div><p class="muted">Recommendation: do not pursue at current pricing. Monitor pending status and only re-engage if premium comps, renovation quality, or meaningful repricing support the number.</p></div>
      <h3>Investor metrics</h3><div class="metric-grid">${metric('Value gap',money(v-p))}${metric('Discount required',pct(requiredDiscount))}${metric('Reported equity',money(d.equity))}${metric('Gross yield @ ask',y?pct(y):'N/A',yieldLine)}${metric('Gross yield @ value',(r&&v)?pct(annualRent/v):'N/A',(r&&v)?`${money(annualRent)} annual rent ÷ ${money(v)} value`:'' )}${metric('Max investor target',money(target))}</div>
      <h3>Calculation trace</h3><div class="card"><ul class="small-list"><li>Monthly rent used: <strong>${money(r)}</strong></li><li>Annual rent used: <strong>${money(annualRent)}</strong></li><li>Price used: <strong>${money(p)}</strong></li><li>Gross yield formula: <strong>${money(annualRent)} ÷ ${money(p)} = ${y?pct(y):'N/A'}</strong></li></ul></div>
      <h3>Investor intelligence</h3><div class="metric-grid">${metric('Value band',money(d.low)+' – '+money(d.high),`Conservative ${money(d.low)} · Supported ${money(v)} · Upper comp screen ${money(d.high)}`)}${metric('Market position','Above-market pricing',`${money(p)} ask vs ${money(d.avgSalePrice)} median reference (${pct(d.avgSalePrice?(p-d.avgSalePrice)/d.avgSalePrice:0)})`)}${metric('Offer framework',money(buyLow)+' – '+money(stretch),`Aggressive ${money(buyLow)} · Reasonable ${money(target)} · Stretch ${money(stretch)}`)}${metric('Seller position','Mortgage '+money(d.mortgage),`At ask: ${money(p-d.mortgage)} equity · At supported value: ${money(v-d.mortgage)} equity · After entered selling costs: ${money(netAtSupported)}`)}${metric('Comp reliability','86/100','Complete CMA packet with summary value, tax value, listing status, map, and nearby sold/listing tables.')}${metric('Renovation premium',pct(premium),'Typical updated-home allowance 5%–15%. Requested premium exceeds normal renovation support.')}</div>
      <h3>Valuation conflict detected</h3><div class="card insight-card"><p><strong>Average comp data supports about ${money(d.avgSalePrice)}.</strong> However, the comp table includes higher renovated or newer nearby sales/listings, so the premium should not be dismissed without hand-selecting the closest 3–5 true comps.</p><p class="muted">DealCalc treats this as a pricing/premium-validation problem: the property may be quality, but the current ask needs strong comparable evidence.</p></div>
      <h3>Best Use Ranking</h3><div class="strategy-grid strategy-grid-explained">${strategies.map((s,i)=>strategyCard(i+1,s[0],s[1],s[2],s[3])).join('')}</div>
      <h3>Verify next</h3><ol class="small-list"><li>Confirm whether the pending listing is still available, under option, or already locked up.</li><li>Hand-pick 3–5 truly comparable sold comps by distance, size, condition, renovation level, and micro-location.</li><li>Verify renovation quality, permits, roof, HVAC, plumbing, electrical age, and whether the premium is justified.</li><li>Run rent against taxes, insurance, vacancy, repairs, management, and actual financing before treating it as a rental candidate.</li>${d.sourceUrl?'<li>Open the source listing URL and compare live price/status against this report.</li>':''}</ol>
      <details class="notes-details"><summary>Why this result was generated</summary><p class="muted">Educational estimate only. DealCalc uses one normalized deal object for price, value, rent, mortgage, equity, and condition before rendering this report.</p></details>
      <p class="cta-row"><button class="btn" onclick="dcSaveCurrentDeal && dcSaveCurrentDeal()" type="button">Save Deal</button><button class="btn secondary" onclick="window.print()">Export / Print Report</button></p><p id="saveDealMsg" class="muted status-line"></p>`;
    if(typeof trackEvent==='function') trackEvent('deal_analyzed',{analysis_type:d.analysis,score:score,manual:manual,source_url:!!d.sourceUrl});
  };
  const oldClear=window.clearAnalyzer;
  window.clearAnalyzer=function(){ if(typeof oldClear==='function') oldClear(); const u=q('listingUrl'); if(u) u.value=''; const r=q('urlImportResult'); if(r) r.remove(); window.dcCurrentListingUrl=''; };
  document.addEventListener('DOMContentLoaded',function(){
    const url=new URLSearchParams(location.search).get('listingUrl');
    if(url && q('listingUrl')){ q('listingUrl').value=url; window.dcCurrentListingUrl=url; updateUrlImportPanel(url); }
  });
})();
/* ===== End DealCalc V35 Reliability Release ===== */
